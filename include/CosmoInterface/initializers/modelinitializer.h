...
namespace TempLat
{
  ...
  public:
    ...
    ModelInitializer(Model &model, T pLSide, std::string pSeed) : fg(model, pLSide, pSeed), extps(model, pLSide, pSeed)
    {
    }
     ...
     */
    template <class Model>
    void initialize(Model &model, RunParameters<T> &rPar, FilesManager<Model::NDim> &filesManager,
