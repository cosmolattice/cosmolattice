...
// File info: Main contributor(s): Daniel G. Figueroa, Adrien Florio, Francisco Torrenti, Ander Urio and Nicolas Loayza Year: 2025
...
namespace TempLat {
    ...
    /** @brief  A class that contains functions related with initial random fluctuations for scalar fields given by an external power spectrum.
     *
     ...
     *  whose amplitudes follow a Gaussian random distribution with variance given by and external power spectrum function.
    */

    template<typename T>
    class ExternalPowerSpectrumInitializer {
    public:
        /* Put public methods here. These should change very little over time. */
        template <class Model>
        ExternalPowerSpectrumInitializer(Model& model, T pLSide, std::string pSeed) :
        lSide(pLSide),
        baseSeed(pSeed)
        {
        ...
        template<class Model>
        void readSpectrumFile(Model& model, const std::string& str, std::vector<T>& kIN, std::vector<T>& PSamp, std::vector<T>& PSampConj) const
        {
            int lineCounter = 0;
            std::ifstream t;
            t.open(str);
            ...
            if(t.good()){
              for (int j = 0; j < lineCounter; j++){
    			         t >> kIN[j]; kIN[j] /= model.omegaStar;      //Now is in omegaStar units, i.e. it's kappa = k/omegaStar
                         t >> PSamp[j]; PSamp[j] *= model.omegaStar;  //Now is in 1/omegaStar units, i.e. \tilde{P} = omegaStar * P(k), e.g. P(k) = 1/(2\omega_k)
    			         t >> PSampConj[j]; PSampConj[j] /= model.omegaStar;  //Now is in 1/omegaStar units, i.e. \tilde{P} = omegaStar * P(k), e.g. P(k) = 1/(2\omega_k)
              }
            }
            t.close();
        }
        ...
        auto getInputFluctuationsNormTypeI(const InterpolatorFunction<NDim, T>& PSinterp, Model& model, Field<NDim, T> f, T kCutOff) const
        {
            
            if(NDim != 3) throw(PSTypeINotSupportedForNDIMDifferentFrom3("Type I external power spectrum initialization currently implemented only for NDim = 3."));
            ...
            return Hcut * (model.omegaStar / model.fStar) * (T(1) / pow(model.aI, T(1.5))) * pow(lSide / pow<2>(f.getDx()), T(1.5)) * safeSqrt(T(4) * Constants::pi<T> * pow(ntilde.norm(), T(2)) * invmult * PSinterp / T(2));
         }

        template<class Model, size_t NDim>
        auto getInputFluctuationsNormTypeII(const InterpolatorFunction<NDim, T>& PSinterp, Model& model, Field<NDim, T> f, T kCutOff) const
        {   
            FourierSite<NDim> ntilde(f.getToolBox());
            auto k = ntilde.norm() * f.getKIR();
            auto Hcut = heaviside(kCutOff - k);

            return Hcut * (model.omegaStar / model.fStar) * (T(1) / pow(model.aI, T(1.5))) * pow(lSide / pow<2>(f.getDx()), T(1.5)) * safeSqrt(PSinterp) / sqrt(T(2));
        }

        template<class Model, size_t NDim>
        auto getInputFluctuationsTypeI(const InterpolatorFunction<NDim, T>& PSinterp, Model& model, Field<NDim, T> f, const std::string& mySeed, T kCutOff) const
        {
            auto norm = getInputFluctuationsNormTypeI<Model, NDim>(PSinterp, model, f, kCutOff);
            return norm * RandomGaussianField<NDim, T>(baseSeed + mySeed + f.toString(), f.getToolBox());
        }

        template<class Model, size_t NDim>
        auto getInputFluctuationsTypeII(const InterpolatorFunction<NDim, T>& PSinterp, Model& model, Field<NDim, T> f, const std::string& mySeed, T kCutOff) const
        {
        ...
        template<class Model, size_t NDim>
        void conjugateGaussianInputFluctuations(Model& model, Field<NDim, T> f, Field<NDim, T> p, const std::string& str, T kCutOff, int PSType) const
        {
    ...
    template<class Model, size_t NDim>
    auto getInputFluctuationsNormTypeI(Model& model, Field<NDim, T> f, T kCutOff) const
    {
        ...
        return Hcut * safeSqrt(T(4) * Constants::pi<T> * pow(ntilde.norm(), T(2)) * invmult) * (T(1) / pow(model.aI, T(1.5))) * pow(lSide / pow<2>(f.getDx()), T(1.5)) * sqrt(0.5);
    }

    template<class Model, size_t NDim>
    auto getInputFluctuationsTypeI(Model& model, Field<NDim, T> f, const std::string& mySeed, T kCutOff) const
    {
    ...
    template<class Model, size_t NDim>
    auto ConjugateGaussianFluctuationsVarianceOne(Model& model, Field<NDim, T> f, Field<NDim, T> p, T kCutOff) const
    {      
    ...
    void BunchDavisTransverseU1(Model& model, VF f, VF p, VF tmpF, VF tmpP, T aDot, T kCutOff) const
    {
            ...
            //definition of the phase shifts to properly have gauge fields living on the links when doing fourier transformations
			...
			//We set the RGF fluctuations with variance = 1 for the gauge and electric field
			      ForLoop(i, 1, Model::NDim,
                        ConjugateGaussianFluctuationsVarianceOne(model, f(i), p(i), kCutOff);
                );
			
            ForLoop(k,1,Model::NDim,
                    f(k).inFourierSpace() = asFourier(extraphaseminus(k)) * f(k).inFourierSpace();
            ...
            ForLoop(k,1,Model::NDim,       
                    tmpF(k).inFourierSpace() = Total(j,1,Model::NDim, ((k == j ? T(1) : T(0)) - keffm(k) * keffm(j) / keffm2) * f(j).inFourierSpace());
                    tmpP(k).inFourierSpace() = Total(j,1,Model::NDim, ((k == j ? T(1) : T(0))  - keffm(k) * keffm(j) / keffm2) * p(j).inFourierSpace());
            );
            
            auto phaseA = BDPhasePi2A<Model::NDim,T>(model.getToolBox(), model.fldS(0_c).getKIR(), model.aDotI);

            auto phaseE = BDPhasePi2E<Model::NDim,T>(model.getToolBox(), model.fldS(0_c).getKIR(), model.aDotI);
             
            /* Initialization of Gauge Field */
            //Sets the purely transverse initial vector field with its Bunch-Davis amplitude and appropiate phase
            // 
            ForLoop(k,1,Model::NDim,
                    f(k).inFourierSpace() = sqrt(1./(2.*knorm)) * phaseA * tmpF(k).inFourierSpace();
            ...
            //
            ForLoop(k,1,Model::NDim,
                    p(k).inFourierSpace() = sqrt(knorm/(2.0)) * phaseE * tmpP(k).inFourierSpace();
...
std::vector<int> getTypeIBinCounts(const Field<NDim, T>& f) const
{
