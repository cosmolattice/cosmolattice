    ...
    template <class KernelType> void evolve(Model &model, T tMinust0, KernelType kt)
    {
      ...
      for (size_t i = 0; i < As.size(); ++i) { // loop over operations...
        ForEachConfigurationField(
            Model, fld, n,
            isDefined[fld][n] = delta(i, model.getField(TempLat::FieldsNumbering::getComplementaryFieldId(fld))(n),
                                      Kernels::get(fld, model, n, kt)););
  ...
  private:
    ...
    template <int N> void advance(size_t i, FieldsNumbering::fldSU2 fld, Model &model, Tag<N> n)
    {
      ForLoop(j, 1, Model::NDim,
              model.fldSU2(n)(j) =
                  exp(Bs[i] * model.getField(TempLat::FieldsNumbering::getComplementaryFieldId(fld))(n)) *
                  model.fldSU2(n)(j););
    }
