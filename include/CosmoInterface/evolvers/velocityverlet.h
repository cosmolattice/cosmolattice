  ...
  public:
    ...
    VelocityVerlet(Model &model, RunParameters<T> &rPar)
        : expansion(rPar.expansion), fixedBackground(rPar.fixedBackground), // A boolean. If true, expansion is fixed
          ...
          ws(VelocityVerletParameters<T>::getCoeffs(
              rPar.eType)), // Vector containing the numerical parameters of the chosen VV method
    ...
    {
      (*model.piGWs) += (w * model.dt / 2) * GWsKernels::get(model);
    }
    ...
    {
      ForLoop(n, 0, Model::NSU2 - 1, model.piSU2(n) += model.dt / 2 * w * SU2Kernels::get(model, n););
    }

    // Evolves aDotSI
    template <class Model> void kickScaleFactorHalf(Model &model, T w)
    {
      model.aDotSI = model.aDotI + (model.dt / 2) * ScaleFactorKernels::get(model) * w;
    }

    // Evolves aDotI
    template <class Model> void kickScaleFactorOne(Model &model, T w)
    {
    ...
    template <class Model> void driftSU2Vector(Model &model, T w)
    {
      ForLoop(n, 0, Model::NSU2 - 1,
              auto rescaledPi = MakeVector(
                  i, 1, Model::NDim,
                  exp(w * pow(model.aSI, model.alpha - 1) * model.dx * model.dt * model.gQ_SU2DblSU2(0_c, n) *
    ...
    template <class Model> void driftScaleFactor(Model &model, T tMinust0, T w)
    {
      ...
      if (fixedBackground) { // if fixed background, the scale factor is given by the power-law function in
        ...
        if constexpr (Model::DefectsModel) {
          if (aBackground.areWeResolutionPreserving())
            ResolutionPreserving::updateResolutionPreservingFactor(model, tMinust0, aBackground.t0, aBackground.tRP0,
                                                                   aBackground.tRPMax, aBackground.sRP);
        }
