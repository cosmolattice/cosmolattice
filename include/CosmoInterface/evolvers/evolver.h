...
namespace TempLat
{
  ...
  template <typename M, typename = void> struct CheckAxionU1 {
  ...
  public:
    ...
    Evolver(Model &model, RunParameters<T> &rPar, ExtraFields<Model> extraFlds)
        : type(rPar.eType), typeGW(rPar.eTypeGW),
          lf((type == LF || (typeGW == LF && model.fldGWs != nullptr)) ? std::make_shared<LeapFrog<T>>(model, rPar)
                                                                       : nullptr),
          ...
          GWsynced(!rPar.doWeRestart)
    {
      ...
      if (Model::NU1 > 0 && rPar.withGWs && type == LF)
        throw(InvalidEvolverTypeGW(
            "The evolution of a model with U(1) and GWs requires to use an evolver that correctly synchronizes fields "
            "and momenta (as both electric and magnetic fields enter the kernel of GWs). You should use a different "
            "evolver for the matter fields (VVn, RK, PV). Abort."));

      if (Model::NU1 > 0 && rPar.withGWs && PositionVerletParameters<T>::isVerlet(type) &&
          PositionVerletParameters<T>::isVerlet(typeGW))
        throw(InvalidEvolverTypeGW(
            "The evolution of a model with U(1) and GWs requires to use an evolver that correctly synchronizes fields "
            "and momenta (as both electric and magnetic fields enter the kernel of GWs). If you want to use PV for the "
            "matter fields, you should use LF for the GWs. Abort."));
      //

      if (CheckAxionU1<Model>::value && !RK2NStorageParameters<T>::isRK2n(type)) {
        throw(InvalidEvolverTypeAxion(
            "The evolution of a model with an Axion-U(1) coupling requires a Runge-Kutta (RK) evolver. Other evolvers "
            "(LF, VV, PV) are not supported for this coupling. Abort."));
      }
    }
    // @endlabel
    ...
    template <int N> void deactivate(Tag<N> t)
    {
      ...
      if (!RK2NStorageParameters<T>::isRK2n(type))
        throw(EvolverTypeNotInEvolver("The activate/desactivate function is implemented only for the RK2N evolvers. Go "
                                      "implement it in the others if you need it."));
    }
