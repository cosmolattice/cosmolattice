  ...
  public:
    ...
    RK2NStorage(Model &model, RunParameters<T> runParams)
        : type(runParams.eType), dt(model.dt), As(RK2NStorageParameters<T>::getAs(type)),
          Bs(RK2NStorageParameters<T>::getBs(type)), Cs(RK2NStorageParameters<T>::getCs(type)),
          expansion(runParams.expansion), fixedBackground(runParams.fixedBackground), aBackground(model, runParams)
    {
    ...
    // This function is called before doing the measurements. It is used to set aI, aDotI and RI to its correct value in
    // case of  fixed-background expansion.
    ...
    // This function is called in the case of NMC evolution to sync the value of R obtained from the volumen averages of
    // the matter fields.
