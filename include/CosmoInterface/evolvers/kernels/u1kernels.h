...
namespace TempLat
{
  ...
  public:
    ...
    template <class Model, int N> static auto get(Model &model, Tag<N> a, KernelsTypes::EoM<Model> eom)
    {
      ...
      auto normGrad =
          pow(model.aI, -1 + model.alpha) / IfElse(Model::DefectsModel, model.resolutionPreservingFactor, OneType());
    ...
    static auto get_momentum(Model &model, Tag<N> a, KernelsTypes::Diffusion<Model> diffusion)
    {
      // Returns momentum for complex scalars:
