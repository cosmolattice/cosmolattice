  ...
  public:
    ...
    template <class Model> static auto get(Model &model, KernelsTypes::EoM<typename Model::FloatType> eom)
    {
      ...
      if constexpr (not Model::IsNonMinimallyCoupled) {
        ...
        if constexpr (Model::IsAxionU1Coupled) NonLinearSwitch = (tMinust0 > model.tNonLinearAxionU1) ? 1.0 : 0.0;
        ...
        return pow(model.aI, 2 * model.alpha + 1) / 3.0 * pow<2>(model.fStar / Model::MPl) *
               ((model.alpha - 2) * (Eks + Ekcs + EkSU2Dbl) + model.alpha * (Egs + Egcs + EgSU2Dbl) +
                (model.alpha - 1) * (NonLinearSwitch * (EelU1 + EmagU1) + EelSU2 + EmagSU2) + (model.alpha + 1) * model.potAvI);
      } else {
