...
namespace TempLat
{
  ...
  /** @brief A class which stores the composition parameters. See appendix in "The art of simulating the early
   * Universe", arXiv:2006.15122
   *
   *
   **/
  template <typename T> struct VelocityVerletParameters {
    ...
    static size_t getOrder(EvolverType ev)
    {
      ...
      if (ev == VV2)
        ...
        throw(NotVVEvolver("You tried to get the accuracy order of a non-velocity verlet evolver, abort."));
      }
      return res;
    }
