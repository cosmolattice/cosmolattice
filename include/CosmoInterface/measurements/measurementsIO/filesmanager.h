...
namespace TempLat
{
  ...
  public:
    FilesManager(ParameterParser &parser, std::string fn, device::memory::host_ptr<MemoryToolBox<NDim>> toolbox,
        ...
        : mToolbox(toolbox), mUseHDF5(pUseHDF5), mUseHDF5Spectra(pUseHDF5Spectra || isUnbinned),
          mPrintHeaders(pPrintHeaders), mAppendMode(pAppendMode), mOverwriteMode(pOverwriteMode), workingDir(fn),
          tag(pTag), flushFreq(pFlushFreq), nMeas(pNMeas), nMeasInfreq(pNMeasInfreq)
    {
#ifdef HAVE_HDF5
    ...
    template <typename T> std::string getSimpleName(const Field<T, NDim> &fld)
    {
      std::string name = fld.toString();
      name = name.erase(name.find("(", 3));
      return name;
