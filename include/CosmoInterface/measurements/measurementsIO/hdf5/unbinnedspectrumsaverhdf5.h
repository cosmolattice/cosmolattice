  ...
  public:
    ...
    UnbinnedSpectrumSaverHDF5(FilesManager<NDim> &fm, const std::string &fn, bool pAmIRoot, bool append,
        ...
        : filename(fm.getHDF5UnbinnedSpectraFn()), verbosity(rPar.spectraVerbosity), nBins(rPar.nBinsSpectra),
          deltaKBin(rPar.deltaKBin), nGrid(rPar.N), kIR(rPar.kIR), uninitialized(true),
          grpName("unbinnedspectra_" + fn), amIRoot(pAmIRoot), appendMode(append), nMeas(fm.getNInfreqMeas()),
          flushCount(0), flushFreq(fm.getFlushFreq())
    {
    }
