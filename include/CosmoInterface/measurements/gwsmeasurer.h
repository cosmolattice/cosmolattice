  ...
  public:
        ...
        : PSType(par.powerSpectrumType)
    {
      ...
      spectraOut.emplace_back(
          SpectrumGWSaver<T>(filesManager, "gws", amIRoot, append, par, !model.fldGWs)); // File for spectra
      // standardOut.emplace_back( MeasurementsSaver<T>(filesManager, "TT_Test", amIRoot, append, getTTHeaders(model))
      // ); // File for TT_check
    }
