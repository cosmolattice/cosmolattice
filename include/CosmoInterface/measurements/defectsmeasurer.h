...
namespace TempLat
{
  ...
  public:
    ...
    DefectsMeasurer(Model &model, FilesManager<Model::NDim> &filesManager, const RunParameters<T> &rPar, bool append,
                    std::string postfix = "", bool onlyMeasureNorm = false)
        : amIRoot(model.getToolBox()->amIRoot()),
          ...
          normOut(filesManager, "norm" + postfix, amIRoot, append, {"t", "norm(phi)", "norm(phi)^2", "var(norm(phi))"},
                  !Model::DefectsModel || (Model::Ns == 0 && Model::NCs == 1)),
          defects(filesManager, "defects" + postfix, amIRoot, append, getDefectHeaders(model),
                  !Model::DefectsModel ||
                      !((Model::Ns > 0 && Model::NCs == 0) || (Model::Ns == 0 && Model::NCs == 1) ||
                        measureDefectsEnergies || measureDefectsStructure)), // Output file for defect observables.
          normSpectraOut(filesManager, "norm" + postfix, amIRoot, append, rPar,
                         !Model::DefectsModel || !(Model::Ns > 0 && Model::NCs == 0))
    {
    }
    ...
    template <class Model> void measure(Model &model, T t)
    {
      ...
      if (measureDefectsEnergies) {
        if constexpr (Model::Ns > 0 && Model::NCs == 0) {
          ForLoop(i, 0, Model::Ns - 1,
                  Ekin += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                          average(Energies::kineticS(model, FieldFunctionals::pi2S(model, i)) *
                                  Weights::weightScalarSinglet(model));
                  Egrad += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                           average(Energies::gradientS(model, FieldFunctionals::grad2S(model, i)) *
                                   Weights::weightScalarSinglet(model)););
          Epot += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                  average(Potential::potential(model) * Weights::weightScalarSinglet(model));
        ...
        if constexpr (Model::NCs > 0 && Model::Ns == 0) {
          ForLoop(i, 0, Model::NCs - 1,
                  Ekin += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                          average(Energies::kineticCS(model, FieldFunctionals::pi2CS(model, i)) *
                                  Weights::weightComplexScalar(model));
                  Egrad += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                           average(Energies::gradientCS(model, FieldFunctionals::grad2CS(model, i)) *
                                   Weights::weightComplexScalar(model)););
          Epot += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                  average(Potential::potential(model) * Weights::weightComplexScalar(model));
          defects.addAverage(Ekin);
          defects.addAverage(Egrad);
          defects.addAverage(Epot);
          if constexpr (Model::NU1 > 0) {
            ForLoop(i, 0, Model::NU1 - 1,
                    EE += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                          average(Energies::electricU1(model, FieldFunctionals::pi2U1(model, i)) *
                                  Weights::weightComplexScalar(model));
                    EB += pow<3>(GetNGrid::get(model) * model.dx * model.aI) *
                          average(Energies::magneticU1(model, FieldFunctionals::B2U1(model, i)) *
                                  Weights::weightComplexScalar(model)););
      ...
      if (measureDefectsStructure) {
        if constexpr (Model::Ns == 1) {
          defects.addAverage(DefectsObservables::computeAreaParameter(model));
        }
        if constexpr (Model::Ns == 2) {
          defects.addAverage(DefectsObservables::computeWindingNumberLengthScalarSinglet(model));
        }
        if constexpr (Model::NCs == 1 && Model::NU1 == 1) {
          defects.addAverage(DefectsObservables::computeWindingNumberLengthComplexScalarU1(model));
        }
      }
    ...
    {
      if constexpr (Model::Ns > 0 && Model::NCs == 0) {
        normSpectraOut.save(lastMeas, t, Total(i, 0, Model::Ns - 1, PSMeasurer.powerSpectrum(model.fldS(i));),
                            pow(model.aI, 2 * model.alpha - 6) *
                                Total(i, 0, Model::Ns - 1, PSMeasurer.powerSpectrum(model.piS(i));));
      }
    }
  ...
  private:
    ...
    template <typename Model> std::vector<std::string> getDefectHeaders(Model &model) const
    {
      ...
      if (measureDefectsStructure) {
        if constexpr (Model::Ns == 1) {
          ret.emplace_back("areaDW");
        }
        if constexpr (Model::Ns == 2 || (Model::NCs == 1 && Model::NU1 == 1)) {
          ret.emplace_back("winfindLengthStrings");
        }
      }
