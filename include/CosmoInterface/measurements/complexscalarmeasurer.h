        ...
        : isSpectraMeasured(createSpectra)
    {
      ...
      ForLoop(
          ...
          standardReOut.emplace_back( // real part
              MeasurementsSaver<T>(filesManager, filesManager.getSimpleName(model.fldCS(i)(0_c)) + postfix, amIRoot,
                                   append, MeansMeasurer::header()));
          standardImOut.emplace_back( // imaginary part
              MeasurementsSaver<T>(filesManager, filesManager.getSimpleName(model.fldCS(i)(1_c)) + postfix, amIRoot,
                                   append, MeansMeasurer::header()));
          ...
          spectraNormOut.emplace_back(SpectrumSaver<T>(filesManager, "norm_cmplx_scalar_" + std::to_string(i) + postfix,
      ...
      if (isSpectraMeasured)
        ForLoop(i, 0, Model::NCs - 1,
                spectraNormOut(i).save(
                    ...
                    pow(model.aI, 2 * model.alpha - 6) * (PSMeasurer.powerSpectrum(model.piCS(i)(0_c)) +
                                                          PSMeasurer.powerSpectrum(model.piCS(i)(1_c)))););
    }
