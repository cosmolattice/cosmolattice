...
namespace TempLat
{
  ...
  public:
    ...
    EnergiesMeasurer(Model &model, FilesManager<Model::NDim> &filesManager, const RunParameters<T> &par, bool append,
                     std::string postfix = "", bool measureEnergyConservation = true)
        : amIRoot(model.getToolBox()->amIRoot()), isEnergyConservationMeasured(measureEnergyConservation),
          ...
          energyCons(filesManager, "energy_conservation", amIRoot, append, getEnergyConsHeaders(),
                     fixedBackground || !isEnergyConservationMeasured) // Output file for checking energy conservation.
    {
    }
