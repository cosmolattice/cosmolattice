...
namespace TempLat
{
  ...
  public:
    ...
    U1Measurer(Model &model, FilesManager<Model::NDim> &filesManager, const RunParameters<T> &par, bool append,
               std::string postfix = "", bool measureGauss = true, bool createSpectra = true)
        : isGaussMeasured(measureGauss), isSpectraMeasured(createSpectra), flagChiralPS(par.flagChiralPS)
    {
      ...
      ForLoop(
          ...
          gauss.emplace_back(
              MeasurementsSaver<T>(filesManager, "gauss_U1_" + std::to_string(i) + postfix, amIRoot, append,
                                   {"t", "av|LHS-RHS|_over_av|LHS+RHS|", "av|LHS|", "av|RHS|"},
                                   !isGaussMeasured)); // Checks the degree of conservation of the U(1) gauss law.

          spectra.emplace_back(
              SpectrumSaver<T>(filesManager, "norm_U1_" + std::to_string(i) + postfix, amIRoot, append, par,
                               !isSpectraMeasured)); // Contains the spectra of the electric and magnetic fields.
          if (flagChiralPS) {
            chiralSpectraA.emplace_back(
                SpectrumSaver<T>(filesManager, "chiral_U1_" + std::to_string(i), amIRoot, append, par));
            chiralSpectraE.emplace_back(
                SpectrumSaver<T>(filesManager, "chiral_Elec_U1_" + std::to_string(i), amIRoot, append, par));
          });
    }
          ...
          if (flagChiralPS) {
            ...
            auto Eplus =
                Total(i, 1, Model::NDim,
                      pow(model.aI, 2 * model.alpha - 2) * PSMeasurer.chiralPowerSpectrumU1(model, k, i, true, false));
            auto Eminus =
                Total(i, 1, Model::NDim,
                      pow(model.aI, 2 * model.alpha - 2) * PSMeasurer.chiralPowerSpectrumU1(model, k, i, false, false));
