...
namespace TempLat
{
  ...
  {
  public:
    UnbinnedPowerSpectrumMeasurer(const RunParameters<T> &par) : PSType(par.powerSpectrumType)
    {
      if constexpr (NDim == 1 || NDim == 2) {
        if (PSType != 2)
          throw(WrongPSType("Power spectra for dimension 1 and 2 is only implemented for PSType = 2. Abort."));
      }
    }
    ...
    {
      return this->powerSpectrum(projectChiralU1Type1(model, u1, c, sign, AorE), GetNGrid::get(model), model.kIR,
                                 model.getToolBox());
    }
    ...
    UnbinnedRadialProjectionResult<T> powerSpectrum(const PS &f, ptrdiff_t N, T kIR, tBox toolBox)
    {
