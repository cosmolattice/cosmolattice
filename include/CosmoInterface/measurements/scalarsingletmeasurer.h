...
namespace TempLat
{
  ...
  public:
        ...
        : ONMeasurer(par), flagON(par.flagON), PSType(par.powerSpectrumType), isSpectraMeasured(createSpectra)
    {
      ...
      ForLoop(
          i, 0, Model::Ns - 1,
          standardOut.emplace_back(MeasurementsSaver<T>(filesManager,
                                                        filesManager.getSimpleName(model.fldS(i)) + postfix, amIRoot,
                                                        append, MeansMeasurer::header()));
          // File for volume-averages
          spectraOut.emplace_back(SpectrumSaver<T>(filesManager, filesManager.getSimpleName(model.fldS(i)) + postfix,
                                                   amIRoot, append, par, !isSpectraMeasured));
          // File for spectra
          ONOut.emplace_back(SpectrumSaver<T>(filesManager, "ON_scalar_" + std::to_string(i) + postfix, amIRoot, append,
                                              par, !(flagON && isSpectraMeasured)));
    ...
    template <typename Model, typename PowerSpectrumMeasurer> // The occupation number is only measured if the user
                                                              // indicates it. The current version requires one
                                                              // additional field to measure it (JBB, Nov 2023).
