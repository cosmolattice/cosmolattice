  ...
  public:
    ...
    Measurer(Model &model, const RunParameters<T> &par, ParameterParser &parser)
        : filesManager(parser, SimulationManager<Model::NDim>::base_filename(par, model), model.getToolBox(),
                       ...
                       par.getFlushFreq(), static_cast<ptrdiff_t>(round((par.tMax - par.t0) / par.tOutFreq)),
          ...
          infreqOutputFreq(
              static_cast<int>(round(par.tOutInfreq / par.dt))), // Number of steps between infrequent output
          rareOutputFreq(static_cast<int>(round(par.tOutRareFreq / par.dt))), // Number of steps between rare output
          verbOutputFreq(
              static_cast<int>(round(par.tOutVerb / par.dt))), // Number of steps between updates in the terminal
          ...
          energySnapshotsMeasurer(model, par, filesManager,
          ...
          PSMeasurer(par), UPSMeasurer(par),
    ...
    template <typename R> void measure(int n, R t, Model &model)
    {
      ...
      if ((n % outputFreq == 0)) {
        ...
        if constexpr (Model::DefectsModel) {
          defectsMeasurer.measure(model, t);
        };
      ...
      if (n % infreqOutputFreq == 0) {
        ...
        if (!unbinnedSpectra) {
          ...
          if constexpr (Model::DefectsModel) {
            defectsMeasurer.measureSpectra(model, t, PSMeasurer);
          };
        } else {
          ...
          // Electric and magnetic spectra, SU(2) gauge sector
          if constexpr (Model::DefectsModel) {
            defectsMeasurer.measureSpectra(model, t, UPSMeasurer);
          };
        }
