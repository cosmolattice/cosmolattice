...
namespace TempLat
{
  template <typename R, typename T, typename Model> class ChiralProjector : public TempLat::BinaryOperator<R, T>
  {
  public:
    ...
    ChiralProjector(const R &pR, const T &pT, const Model &model, int component, bool sign)
        : TempLat::BinaryOperator<R, T>(pR, pT), mComponent(component), mSign(sign),
          mN(static_cast<S>(GetNGrid::get(model))), mR1(mR(1_c)), mR2(mR(2_c)), mR3(mR(3_c))
    {
    }
  ...
  private:
    ...
    template <typename X> DEVICE_FORCEINLINE_FUNCTION complex<S> extraPhaseMinus(const X &n) const
    {
      const S phase = -Constants::pi<S> * static_cast<S>(n) / mN;
      return complex<S>(cos(phase), sin(phase));
    }

    template <typename X> DEVICE_FORCEINLINE_FUNCTION S kEff(const X &n) const
    {
      return sin(Constants::pi<S> * static_cast<S>(n) / mN);
    }

    DEVICE_FORCEINLINE_FUNCTION S delta(int a, int b) const { return a == b ? S(1) : S(0); }
  ...
  public:
    ...
    DEVICE_FORCEINLINE_FUNCTION auto eval(const IDX &...idx) const
    {
      ...
      auto k = DoEval::eval(mT, idx...);

      device::array<complex<S>, 3> u;
      u[0] = DoEval::eval(mR1, idx...);
      u[1] = DoEval::eval(mR2, idx...);
      u[2] = DoEval::eval(mR3, idx...);
  ...
  auto projectChiralU1Type1(const Model &model, Tag<U1>, Tag<C>, bool sign, bool AorE)
  {
    ...
    if (AorE) {
      return ChiralProjector(MakeVector(i, 1, Model::NDim, model.fldU1(Tag<U1>())(i).inFourierSpace()),
                             WaveNumber(model.getToolBox()), model, C - 1, sign);
    } else {
      return ChiralProjector(MakeVector(i, 1, Model::NDim, model.piU1(Tag<U1>())(i).inFourierSpace()),
                             WaveNumber(model.getToolBox()), model, C - 1, sign);
    }
  }
