...
namespace TempLat
{
  ...
  public:
    ...
    template <class Model, int N> static inline auto derivS(Model &model, Tag<N> n)
    {
    ...
    // Returns a complex number composed by the first derivatives of the potential wrt components 0 and 1 of the complex
    // scalar "n".
    ...
    // Returns a complex number composed by the second derivatives of the potential wrt components 0 and 1 of the
    // complex scalar "n".
    template <class Model, int N> static inline auto deriv2CS(Model &model, Tag<N> n)
    {
      return Complexify(
    ...
    template <class Model, int N> static inline auto derivSU2Doublet(Model &model, Tag<N> n)
    {
      return MakeSU2Doublet(a, derivComponentFromNorm(model.potDerivNormSU2Doublet(n), model.fldSU2Doublet(n), a));
    }

    // Returns a doublet composed by the second derivatives of the potential wrt the components of the SU2 doublet "n".
    template <class Model, int N> static inline auto deriv2SU2Doublet(Model &model, Tag<N> n)
    {
