...
namespace TempLat
{
  ...
  public:
    ...
    template <class Model> static auto F(Model &model)
    {
      auto sum = Total(i, 0, Model::Ns - 1,
                       IfElse(Model::NonMinimalCouplings::couples(i, Tag<0>()),
                              (6.0 * model.xis(i, 0_c) - 1.0) * model.xis(i, 0_c) * pow<2>(model.fStar / model.MPl) *
                                  model.fld2AvSI_i(i),
                              ZeroType()));
      return 1.0 / (1.0 + sum);
    }

    template <class Model> static auto R(Model &model)
    {
      ...
      auto rhoMinimal =
          Eks + Ekcs + EkSU2Dbl + Egs + Egcs + EgSU2Dbl + EelU1 + EmagU1 + EelSU2 + EmagSU2 + model.potAvI;
      ...
      auto nonMinimalContribution = Total(i, 0, Model::Ns - 1,
                                          IfElse(Model::NonMinimalCouplings::couples(i, Tag<0>()),
                                                 12 * model.xis(i, 0_c) * (model.pi2AvSI_i(i) - model.grad2AvSI_i(i)) -
                                                     6.0 * model.xis(i, 0_c) * model.fldVpAvSI(i),
                                                 ZeroType()));
