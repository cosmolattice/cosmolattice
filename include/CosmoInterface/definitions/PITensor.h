...
namespace TempLat
{
  ...
  public:
    ...
    static inline auto effectiveAnisotropicTensor(Model &model)
    {
      return ConstructSymTraceless(
          effectiveAnisotropicTensor(model, 1_c, 1_c), effectiveAnisotropicTensor(model, 1_c, 2_c),
          effectiveAnisotropicTensor(model, 1_c, 3_c), effectiveAnisotropicTensor(model, 2_c, 2_c),
          effectiveAnisotropicTensor(model, 2_c, 3_c), effectiveAnisotropicTensor(model, 3_c, 3_c));
    }
  ...
  private:
    ...
    {
      return scalarSingletContribution(model, i, j) + complexScalarContribution(model, i, j) +
             electricU1Contribution(model, i, j) + magneticU1Contribution(model, i, j);
    }
    // @endlabel
