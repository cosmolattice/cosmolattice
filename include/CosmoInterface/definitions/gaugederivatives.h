...
namespace TempLat
{
  ...
  public:
    ...
    template <class Model, int N, int I> static auto forwardCovGradientSU2Doublet(Model &model, Tag<N> n, Tag<I> i)
    {
      return (SU2sForSU2DoubletCovDerivs(model, n, i) *
                  (U1sForSU2DoubletCovDerivs(model, n, i) * shift<I>(model.fldSU2Doublet(n))) -
              model.fldSU2Doublet(n)) /
             model.dx;
    }
    ...
    template <class Model, int N, int I>
    static auto U1sForSU2DoubletCovDerivs(Model &model, Tag<N> n,
                                          Tag<I> i) // N is the matter index and i is the spatial dimension
    {
      return fold_multiply(
          MakeArray(a, 0, Model::NU1 - 1,
                    IfElse(Model::SU2DoubletU1Couplings::couples(Tag<N>(), a),
                           complexPhase(-model.dx * model.gQ_SU2DblU1(n, a) *
    ...
    template <class Model, int N> static auto forwardGradient(Model &model, Tag<N> n)
    {
      return MakeVector(i, 1, Model::NDim, forwardGradient(model, n, i));
    }

    template <class Model, int N> static auto forwardCovGradientSU2Doublet(Model &model, Tag<N> n)
    {
      return MakeVector(i, 1, Model::NDim, forwardCovGradientSU2Doublet(model, n, i));
    }

    template <class Model, int N> static auto forwardCovGradientCS(Model &model, Tag<N> n)
    {
      return MakeVector(i, 1, Model::NDim, forwardCovGradientCS(model, n, i));
    }

    template <class Model, int N> static auto U1sForCSCovDerivs(Model &model, Tag<N> n)
    {
      return MakeVector(i, 1, Model::NDim, U1sForCSCovDerivs(model, n, i));
    }

    template <class Model, int N> static auto U1sForSU2DoubletCovDerivs(Model &model, Tag<N> t)
    {
      return MakeVector(i, 1, Model::NDim, U1sForSU2DoubletCovDerivs(model, t, i));
    }

    template <class Model, int N> static auto SU2sForSU2DoubletCovDerivs(Model &model, Tag<N> t)
    {
