  ...
  public:
      ...
      return 0.5 * pow<2>(model.omegaStar / model.fStar) * pow<-4>(model.aI) * fldf *
             IfElse(Model::DefectsModel, model.resolutionPreservingFactor,
                    OneType()); // In case we perform a simulatin with defects that uses resolution-preserving
                                ...
                                // multiply (rather than dividing) to compensate the 1/fatteingFactor^2 within the
    ...
    template <class Model> static inline auto rhoNMCAv2(Model &model)
    {
      auto rhoNMC2 = Total(
          i, 0, Model::Ns - 1,
          IfElse(Model::NonMinimalCouplings::couples(i, 0_c),
                 3 * model.xis(i, 0_c) * (2 * pow(model.aI, -model.alpha - 4) * model.aDotI * model.fldPiAvSI[i]), 0));
      return rhoNMC2;
    }

    template <class Model> static inline auto rhoNMCAv(Model &model) { return rhoNMCAv1(model) + rhoNMCAv2(model); }
    ...
    template <class Model> static inline auto rhoMinimal(Model &model)
    {
      ...
      return (Eks + Ekcs + EkSU2Dbl + Egs + Egcs + EgSU2Dbl + EelU1 + EmagU1 + EelSU2 + EmagSU2 + model.potAvI);
    }
    ...
    template <class Model> static inline auto rho(Model &model) // Total energy density (sum of all contributions)
    {
      ...
      auto ENMC = [&]() {
        if constexpr (Model::IsNonMinimallyCoupled)
          return rhoNMCAv(model);
        else
          return ZeroType();
      }();

      return (Eks + Ekcs + EkSU2Dbl + Egs + Egcs + EgSU2Dbl + EelU1 + EmagU1 + EelSU2 + EmagSU2 + ENMC + model.potAvI);
    }
    ...
    static inline auto totalEnergy(Model &model) // Total energy density (sum of all contributions)
    {
      ...
      ForLoop(i, 0, Model::Ns - 1, Etotal += kineticS(model, FieldFunctionals::pi2S(model, i));
              Etotal += gradientS(model, FieldFunctionals::grad2S(model, i)););
      ForLoop(i, 0, Model::NCs - 1, Etotal += kineticCS(model, FieldFunctionals::pi2CS(model, i));
      ...
      ForLoop(i, 0, Model::NU1 - 1, Etotal += electricU1(model, FieldFunctionals::pi2U1(model, i));
              Etotal += magneticU1(model, FieldFunctionals::B2U1(model, i)););
      ForLoop(i, 0, Model::NSU2 - 1, Etotal += electricSU2(model, FieldFunctionals::pi2SU2(model, i));
