...
namespace TempLat
{
  ...
  // WARNING: This class is a slow and memory expensive implementation of the transversality and tracelessness checks of
  // GWs. It can be used by uncommenting the relevant lines in "measure.h" and "gwsmeasurer.h" files. A more optimized
  // version will be included in the future. The output file contains the time, the three components of the
  // transversality check (lambda) and the tracelessness check. For more info see the GW technical note in
  // cosmolattice.net
  class CheckTT
  {
  public:
    template <typename T> CheckTT(const RunParameters<T> &par) : PRJType(par.GWprojectorType) {}
    ...
    template <class Model, int I, int J, typename T = double> inline auto PrCF(Model &model, Tag<I> i, Tag<J> j)
    {
      ...
      auto expIK =
          MakeVector(k, 1, Model::NDim, complexPhase(-2.0 * Constants::pi<T> / N * ntilde(k))); // e^(-2*pi*k/N)
      auto keffm = MakeVector(k, 1, Model::NDim, 1_c - expIK(k));
    ...
    template <class Model, int I, int J, typename Q = double> auto getProjectedFieldR(Model &model, Tag<I> i, Tag<J> j)
    {
      ...
      Field<Q, Model::NDim> AuxFld("AuxFld", toolBox);
    ...
    template <typename Model, int I> auto checkTrans(Model &model, Tag<I> i)
    {
      ...
      if (PRJType == 2) {
        avgGrad = average(abs(backDiff(getProjectedFieldC(model, i, 1_c), 1_c) +
                              backDiff(getProjectedFieldC(model, i, 2_c), 2_c) +
                              backDiff(getProjectedFieldC(model, i, 3_c), 3_c)));
        avgDihij = average(abs(forwDij(getProjectedFieldC(model, i, 1_c), 1_c) +
                               forwDij(getProjectedFieldC(model, i, 2_c), 2_c) +
                               forwDij(getProjectedFieldC(model, i, 3_c), 3_c)));
      } else if (PRJType == 3) {
        avgGrad = 2 * average(abs(forwDiff(getProjectedFieldC(model, i, 1_c), 1_c) +
                                  forwDiff(getProjectedFieldC(model, i, 2_c), 2_c) +
                                  forwDiff(getProjectedFieldC(model, i, 3_c), 3_c)));
        avgDihij = average(abs(forwDij(getProjectedFieldC(model, i, 1_c), 1_c) +
                               forwDij(getProjectedFieldC(model, i, 2_c), 2_c) +
                               forwDij(getProjectedFieldC(model, i, 3_c), 3_c)));
      } else {
        avgGrad = 2 * average(abs(neutDiff(getProjectedFieldR(model, i, 1_c), 1_c) +
                                  neutDiff(getProjectedFieldR(model, i, 2_c), 2_c) +
                                  neutDiff(getProjectedFieldR(model, i, 3_c), 3_c)));
        avgDihij = average(abs(neutDij(getProjectedFieldR(model, i, 1_c), 1_c) +
                               neutDij(getProjectedFieldR(model, i, 2_c), 2_c) +
                               neutDij(getProjectedFieldR(model, i, 3_c), 3_c)));
      }
    ...
    template <typename Model> auto checkTrace(Model &model)
    {
      ...
      if (PRJType == 2 || PRJType == 3) {
        Trhij = average(abs(getProjectedFieldC(model, 1_c, 1_c) + getProjectedFieldC(model, 2_c, 2_c) +
                            getProjectedFieldC(model, 3_c, 3_c)));
        Trhij2 = average(abs(getProjectedFieldC(model, 1_c, 1_c)) + abs(getProjectedFieldC(model, 2_c, 2_c)) +
                         abs(getProjectedFieldC(model, 3_c, 3_c)));
      } else {
        Trhij = average(abs(getProjectedFieldR(model, 1_c, 1_c) + getProjectedFieldR(model, 2_c, 2_c) +
                            getProjectedFieldR(model, 3_c, 3_c)));
        Trhij2 = average(abs(getProjectedFieldR(model, 1_c, 1_c)) + abs(getProjectedFieldR(model, 2_c, 2_c)) +
                         abs(getProjectedFieldR(model, 3_c, 3_c)));
      }
      return Trhij / Trhij2;
    }
