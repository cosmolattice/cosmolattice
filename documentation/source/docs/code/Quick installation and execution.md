...
!!! note
    ...
    Some of the above commands are only indicative, as they can change from machine to machine. For more detailed explanations about the installation of $\mathcal{C}\mathtt{osmo}\mathcal{L}\mathtt{attice}$ and the libraries it uses (or can use), see Appendix [Installation](../code/Installation.md).
