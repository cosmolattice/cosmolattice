...
```
where $\alpha$ is a real number conveniently chosen for a given problem at hand. We denote it as **$\alpha$-time**, and it represents cosmic and conformal time for $\alpha=0,1$, respectively. In what the following, we will keep $\alpha$ as an unspecified real number, writing all the relevant equations in terms of $\alpha$-time. 

The evolution of the scale factor is dictated by the stress-energy tensor of the matter fields, which in order to be compatible with homogeneity and isotropy, must take the following $perfect fluid$ form,
[](){ #eq_stresstensor }
```math
...
In Section [Lattice-Cosmology Reviews](MonographicReviews.md) we present the lattice formulation of different theories involving scalar and/or gauge fields, based on the techniques introduced in [Field Discretization Primer](Brief_Review_On_Lattice_Techniques.md). In order to introduce notation, 
 -->
We consider now the equations of motion in a scalar-gauge theory with three kinds of canonically-normalized scalar fields: a real scalar singlet $\phi$, a $U(1)$-charged complex scalar field $\varphi$, and a $[SU(N) \times U(1)]$-charged scalar field $\Phi$. In the last two cases, there are also present, respectively, Abelian and non-Abelian gauge fields, $A_{\mu}$ and $C_{\mu} \equiv C_{\mu}^a T_a$, with $T_a$ the $N^2-1$ group generators of $SU(N)$. The scalar fields can be explicitly written in terms of real components as follows,
[](){ #eq_ChargedScalars }
```math
...
\end{align}
```
with $V \equiv V(\phi,|\varphi|, |\Phi|)$ the potential describing the interactions between the scalar fields, and where the *covariant derivatives* and *field strength tensors* associated to the gauge fields, are defined as
[](){ #eq_AbCovDerivCont }
[](){ #eq_CovDerivCont }
```math
\begin{eqnarray}
...
\begin{eqnarray}
    ...
    \varphi'' - a^{-2(1 - \alpha)} {\vev D}_{\hspace{-0.5mm}A}^{\,2}\varphi + (3 - \alpha) \mathcal{H}  {\varphi'} &=& - \frac{a^{2 \alpha}V_{,|\varphi|} }{2} \frac{\varphi}{|\varphi |} \ , \label{eq_higgsU1-eom}\\
