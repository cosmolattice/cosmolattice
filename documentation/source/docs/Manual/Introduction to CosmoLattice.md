...
```math
\begin{align}
\class{cl-tag-scalar}{\ddot{\phi}} - a^{-2} {\vec\nabla}^{ 2} \hspace{-1mm}\phi + 3 \frac{\dot{a}}{a} \dot{\phi} &= - V_{,\phi}  , \tag{4} \\[0.9em]
\class{cl-tag-u1s}{\ddot{\varphi}} - a^{-2} {\vec D}_{\hspace{-0.5mm}A}^{ 2}\varphi + 3\frac{\dot{a}}{a}  {\dot \varphi} &= - {1\over2}{\varphi \over |\varphi|}V_{,|\varphi|}  , \tag{5}\\[0.9em]
\class{cl-tag-su2s}{\ddot{\Phi}} - a^{-2} {\vec D}^{ 2}\Phi + 3 \frac{\dot a}{a}  {\dot{\Phi}} &= - {1\over2}{\Phi \over |\Phi |}V_{,|\Phi|}  , \tag{6}
\\[0.9em]
\class{cl-tag-u1g}{\partial_0 F_{0i}} - a^{-2} \partial_j F_{ji} +  \frac{\dot a}{a} F_{0i} &=
J^A_i  , \tag{7}
\\[0.9em]
\class{cl-tag-su2g}{(\mathcal{D}_0 )_{a b} (G_{0i})^b} -  a^{-2} ( \mathcal{D}_j )_{a b} (G_{ji} )^b + \frac{\dot a}{a} (G_{0i} )^b &= (J_i)_a  , \tag{8}
\end{align}
```
...
```math
\begin{align}\tag{9}
\class{cl-panel-gauss}{\partial_i F_{0i}} &= a^2J^A_0 , \\ \tag{10}
...
[](){ #eq_Friedmann-full }
```math
\hspace{0.6cm} \class{cl-panel-friedmann}{{\ddot a\over a}} = - \frac{1}{6 m_p^2}[ \bar\rho + 3 \bar p ] ,\tag{11}
```
...
```math
\tag{12}
\class{cl-panel-hubble}{H^2} \equiv \left({\dot a\over a}\right)^2 =  \frac{\bar\rho}{3 m_p^2}  ,
```
...
```math
\tag{15}
\class{cl-panel-fixed}{a(t)} = a (t_* ) \left(1 + \frac{3 (1 + w)}{2} H (t_*) (t-t_*) \right)^{\frac{2}{3(1 + \omega) }  , }  ,
```
