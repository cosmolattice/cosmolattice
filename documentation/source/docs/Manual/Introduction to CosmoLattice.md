...
CosmoLattice can handle scenarios that include singlet scalar fields, scalar fields charged under a $U(1)$ and/or $SU(2)$ gauge symmetry, and the corresponding Abelian and/or non-Abelian gauge vector fields, axion-like-particles (ALP) interacting with gauge sectors, non-minimal scalar field dynamics, and soon enough (see below) fluid dynamics. CosmoLattice can simulate the dynamics of such fields either in a flat space-time background, or in a homogeneous and isotropic (spatially flat) expanding background. In the latter case, the fields can evolve either over a fixed background (e.g. with a power-law scale factor), or **self-consistently, with the fields involved in a simulation determining themselves the expansion rate of the universe**. 
...
We welcome everyone to use CosmoLattice for their own projects. Whenever using CosmoLattice in your research, no matter how much (or little) you modify or use the code, and indepenently of which part of the code you use, please cite the basic versions of the code (version 1.0 and version 2.0) as well as our lattice technique dissertations $\mathtt{The~Art-I}$[@Figueroa_2020rrl] and $\mathtt{The~Art-II}$[@Baeza-Ballesteros:2025tme], where the basic algorithms and techniques implemented in CosmoLattice are explained. For example, if you have used only fundamental fields (either scalar or gauge fields, or both), you could use a line as follows:

*"In this work we use CosmoLattice~\cite{Figueroa:2021yhd,Baeza-Ballesteros:2026XYZ}, based on the lattice-cosmology techniques presented in~\cite{Figueroa_2020rrl,Baeza-Ballesteros:2025tme}*.

and, in case of having added new features, *e.g.* some new dynamics, a specific observable, etc, then:

*"In this work we use CosmoLattice~\cite{Figueroa:2021yhd,Baeza-Ballesteros:2026XYZ}, based on~\cite{Figueroa_2020rrl,Baeza-Ballesteros:2025tme}. We have modified the module [...] so that the code can run the dynamics of [...] / produce a new observable based on [...]"*.

**If you have used the fluid module**, then the above acknowledgements should rather read as
...
or, in case of adding new features (*e.g.* some new dynamics, a specific observable, etc)

*"In this work we use CosmoLattice~\cite{Figueroa:2021yhd,Baeza-Ballesteros:2026XYZ,Figueroa:2026XYZ}, based on~\cite{Figueroa_2020rrl,Baeza-Ballesteros:2025tme,Figueroa:2026RST}. We have modified the module [...] so that the code can accommodate for the dynamics of [...] / a new observable based on [...]"*.
