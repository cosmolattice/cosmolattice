...
| fftw3, parallel version                  | 3               | Parallelization in one dimension                                                                              | `fftw3` compiled for parallel use. Allows only for parallelization in one dimension.                                                                                                                                                                        |
...
\captionof{table}{Summary of required and optional tools and libraries related to CosmoLattice. See main text for more information. Note that the code should in theory work with a recent (at least above version `17.0`) `intel` compiler as well, but this has not been extensively tested. }
