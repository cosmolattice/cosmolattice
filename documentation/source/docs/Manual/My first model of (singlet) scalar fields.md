...
cd build
```
$\mathcal{C}\mathtt{osmo}\mathcal{L}\mathtt{attice}$ uses CMake for compilation (see Section [CMake Flags](../code/cmake.md) for more details). The model $\texttt{lphi4.h}$ is compiled by typing the following commands,
```bash
cmake -DMODEL=lphi4 ../
make cosmolattice
```
...
| `dx`                                     | Lattice spacing **in program units**, i.e. $\delta \tilde{x} \equiv \delta x \, \omega_*$. The size of the lattice can be fixed with any of `kIR`, `lSide` or `dx`: at least one must be given, and if several are given they must satisfy $\tilde{L} = 2\pi / \tilde{k}_{\rm IR} = N \delta \tilde{x}$.                                                                                                                                                                                                                                                                     |
...
| `evolver    `                            | Type of evolution algorithm. For example, options `VV2`, `VV4`, `VV6`, `VV6_2`, `VV8`, `VV10`, solve the field equations with the *velocity-verlet* algorithm of order 2, 4, 6, 8, or 10, respectively, while `LF` solves the equations of motion with the second-order method *staggered-leapfrog*. Other evolvers are `PV2`, `PV4`, `PV6`, `PV6_2`, `PV8`, `PV10` for *position-verlet* and `RK2`, `RK3_3`, `RK3_4`, `RK4_5` for *Runge-Kutta*. This list might change in time if new evolvers are added. For an exhaustive list of evolvers check [*Run Parameters*][subsec_runParam] . |
...
-   `average_energies.txt`{ .cl-fname }
    ...
    Energy density volume-averaged components in the following order: 
...
where $\tilde k \equiv k/\omega_*$, the dimensionless power spectra are related to their dimensionful counterparts [see Eq. ([*20*][eq_discretePST1]) in  [*Field Discretization Primer*](../th_framework/Brief_Review_On_Lattice_Techniques.md) of the [*Theory Framework*](../th_framework/TF_readme.md) Tab] by $\Delta_{\phi} \equiv \widetilde \Delta_{\tilde\phi}f_*^2$, and $\Delta_{\phi'} \equiv \widetilde\Delta_{\tilde\phi'}f_*^2\omega_*^2$, $\Delta n_{bin}$ is the *multiplicity* (*i.e.* lattice sites/bin), and we have defined a (dimensionless) *lattice occupation number* as
[](){ #eq_OccuppationNum }
```math
\begin{equation}
%\tag{44}
...
Here, we use the default function which sets the masses from the second derivatives of the potential evaluated on the initial homogeneous values of the field, see Section [*The potential and its derivatives*][sec_PotDerivs] below. This function also computes the initial value of the potential, which is useful to initialise the Hubble rate. If the user prefers instead to provide explicitly the initial field masses, the relevant parameter to be set is `masses2S`, which represents an array containing the square masses of the scalar fields (evaluated on the initial homogeneous values of the field).
...
\delta \tilde \phi ({  \bf \tilde{n}}) &= \frac{1}{\sqrt{2}} (\delta \tilde \phi_1 ({  \bf \tilde{n}}) + \delta \tilde \phi_2 ({   \bf \tilde{n}})  ) %\tag{47}  
, \\
