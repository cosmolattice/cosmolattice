...
The keyword `auto` allows the compiler to automatically deduce the return type. In our case, it is essential as the expression we return are symbolic expression encoded inside the type, through the mechanism known as *''expression templates"*. We defer the interested reader to Appendix [Under the Hood: Expression Templates and CosmoLattice](Under the Hood: Expression Templates and CosmoLattice.md) for more information. For related reasons, the syntax `1_c` with the unusual `_c`, is needed as it allows to simply defined compile-time integer.
