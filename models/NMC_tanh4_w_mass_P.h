...
namespace TempLat
{
  ...
  struct ModelPars : public TempLat::DefaultModelPars {
    ...
    // Non-minimal coupling: the second scalar (index 1) couples non-minimally to gravity. The first scalar (index 0)
    // only couples minimally to gravity.
...
#define MODELNAME NMC_tanh4_w_mass_P
  ...
  // Declaration of our model. It inherits from the generic model defined above.
  {
    //...
  private:
    FloatType lambdaNMC, mNMC, Hstar, V0, M;
  ...
  public:
                           ...
                           STRINGIFY(MODELLABEL)) // MODELLABEL is defined in the cmake.
    {
      ...
      /////////
      Hstar = parser.get<FloatType>(
          "Hstar"); // Hubble rate 14 efolds before the end of inflation, begin of linear analysis in GeV
      V0 = parser.get<FloatType>("V0");               // Lambda4/(4 * mp^2 * Hstar^2)
      M = parser.get<FloatType>("M");                 // M in units of mp
      lambdaNMC = parser.get<FloatType>("lambdaNMC"); // Quartic Coupling of the NMC field
      mNMC = parser.get<FloatType>("mNMC");           // Mass of the NMC field in units of Hstar.
    ...
    {
      return FloatType(0.5) * pow<2>(mNMC) * pow<2>(fldS(1_c)) +
             FloatType(0.25) * pow<2>(MPl / Hstar) * lambdaNMC * pow<4>(fldS(1_c));
    }
    ...
    {
      return V0 * 4 * (1 / M) * pow<3>(tanh(fldS(0_c) / M)) / pow<2>(cosh(fldS(0_c) / M));
    ...
    {
      return V0 * 4 * pow<2>(1 / M) * (4 - cosh(2 * fldS(0_c) / M)) * pow<2>(tanh(fldS(0_c) / M)) /
             pow<4>(cosh(fldS(0_c) / M));
      // return 3.0 * pow<2>(fldS(0_c));
    }

    auto potDeriv2(Tag<1>) // NMC field potential 2nd deriv
    {
      return pow<2>(mNMC) + 3 * pow<2>(MPl / Hstar) * lambdaNMC * pow<2>(fldS(1_c));
    }
  };
} // namespace TempLat
