...
bool test_roundtrip(MPI_Comm comm) {
    ...
    parafaft::ParaFaFT_R2C<3> fft(global_shape, comm);
    ...
    MPI_Barrier(comm);
    std::cout << "Rank " << rank << ": real shape = [" << local_real_shape[0] << ", "
              << local_real_shape[1] << ", " << local_real_shape[2] << "]"
              << ", real start = [" << real_start[0] << ", " << real_start[1] << ", " << real_start[2] << "]"
              << ", complex shape = [" << local_complex_shape[0] << ", "
              << local_complex_shape[1] << ", " << local_complex_shape[2] << "]"
            ...
            for (int k = 0; k < local_complex_shape[2]; ++k) {
                ...
                if (error > TOLERANCE * total_N) {
                    if (rank == 0 || (gi == 0 && gj == 0 && gk == 0)) {
                        std::cout << "Rank " << rank << ": k=(" << gi << "," << gj << "," << gk
                                  << "): got " << complex_output[idx]
                                  << ", expected " << expected
                                  << ", error = " << error << std::endl;
                    }
                    passed = false;
