...
namespace TempLat {
  ...
  for (ptrdiff_t i = 0; i < nDim; ++i) {
    ...
    gradientFFT.back().inFourierSpace() =
        (exp(complex<double>(0, 1) * k[i] * 2 * pi * dx) - 1.) / dx * gaussian.inFourierSpace();
  ...
  for (it.begin(); it.end(); ++it) {
    ...
    for (ptrdiff_t i = 0; i < nDim; ++i) {
      ...
      if (!thisResultPart) {
        ...
        say << "grad " << i << ", at " << coordsStr << ": " << gradientFiniteDiff[i].get(it()) << " vs "
            << gradientFFT[i].get(it()) << "\n";
      }
...
namespace
{
  TempLat::TDDContainer<TempLat::FiniteDiffVSFFT> test;
}
