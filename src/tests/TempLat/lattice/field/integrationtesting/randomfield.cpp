...
#include "TempLat/lattice/field/integrationtesting/randomfield.h"
...
namespace TempLat
{
  ...
  template <size_t nDim> void testHelper(auto &tdd, ptrdiff_t nGrid, ptrdiff_t nGhost, bool forbidTransposition)
  {
    ...
    for (it.begin(); it.end(); ++it) {
      ...
      //        sayMPI << pIterCoords << ", " << HP->qualify(pIterCoords.getCoordinates()) << " a: " << a << ", b: " <<
      //        b << "\n";
  ...
  namespace IntegrationTesting
  {
    ...
    void RandomField::Test(TDDAssertion &tdd)
    {
      say << "Testing C2R -> R2C with a random gaussian. Actually more a test of HermitianPartners and "
             "FFTNormalization "
             "than anything else.\n";
      ...
      //                    if ( ! goOn ) {
      //                        sayMPI << "Interrupting RandomField::Test, after failing at  myLittleLambda(" << iDim <<
      //                        ", " << 4 + 20 * iSize << ", " << iGhost << ", " << iTrans << "); \n";
      //                    }
      //                }
      //            }
      //        }
...
namespace
{
  TempLat::TDDContainer<TempLat::IntegrationTesting::RandomField> test;
}
