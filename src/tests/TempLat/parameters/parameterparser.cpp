...
struct ParameterParserTester {
  static void Test(TDDAssertion &tdd);
};

void ParameterParserTester::Test(TDDAssertion &tdd)
{
  ...
  /*char t1[256] ("aaa=bbb"), t2[256]("  aa = 25.78"), t3[256]("cc=true"), t4[256]("d= 24 45 76 143"),
 t5[256]("input=../../src/tests/TempLat/parameters/test_filereader.txt"); char* argv[6]; argv[0] = t1; argv[1] = t1;
  argv[2] = t2;
  argv[3] = t3;
  argv[4] = t4;
  argv[5] = t5;
  ParameterParser test(6, argv);// we pass number of string plus 1 to argc because in real situation ther eis also the
 program name std::cout<<"Parameters: "<<test;
  ...
  auto myOptInt5 = test.get<int, 0>("iii5"); //Just to make sure it does not crash even if mandatory. Useful if the size
 of array depends on templated size that can be zero.
