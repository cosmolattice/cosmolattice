  ...
  public:
                                                    ...
                                                    bool forbidTransposition = PFFTWITHTRANSPOSITION())
    {
      if (!forbidTransposition && !PFFTWITHTRANSPOSITION()) {
        ...
        if (group.getRank() == 0)
          say << KBLU "Sorry, currently only the un-transposed version of PFFT works. Forcing forbidTransposition = "
                      "true now. This is slightly slower, but if FFTs are only done in the startup phase and an "
                      "occasional measurement, they should not be the bottle neck. Want to work on this issue? See "
                      "TempLat/lattice/field/integrationtesting/randomfield_test.h. That test fails if you enable "
      ...
      result.addExternalMemoryRequest(2 *
                                      pfftRequiredMemory); /* pfftRequiredMemory is in unit of complex numbers,
