...
namespace TempLat
{
  ...
  public:
    ...
    friend std::ostream &operator<<(std::ostream &ostream, const PFFTTranspositionFlags &tr)
    {
      ostream << "C2R : "
              << ((tr.mFlagC2R & PFFT_TRANSPOSED_IN)    ? " transposed in"
                  : (tr.mFlagC2R & PFFT_TRANSPOSED_OUT) ? "transposed out"
                                                        : "not transposed")
              << ".\n";
      ostream << "R2C : "
              << ((tr.mFlagR2C & PFFT_TRANSPOSED_IN)    ? " transposed in"
                  : (tr.mFlagR2C & PFFT_TRANSPOSED_OUT) ? "transposed out"
                                                        : "not transposed")
              << ".\n";
      return ostream;
    }
