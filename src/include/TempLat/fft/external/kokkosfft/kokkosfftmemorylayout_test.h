...
inline void TempLat::KokkosFFTMemoryLayout::Test(TempLat::TDDAssertion &tdd)
{
  MPICommReference world;

  //    KokkosFFTMemoryLayout mem; // cannot do this. Need the descending class for that.
  KokkosFFTInterface mem;
  ...
  auto &&computeExpectation = [&](const MPICartesianGroup &mGroup, const std::vector<ptrdiff_t> &nGrid) {
    ...
    if (nDim > 1) {
      ...
      for (ptrdiff_t i = 0; i < (ptrdiff_t)decomposition.size(); ++i) {
        ...
        fourLocalStarts[fourTransposition[i]] =
            (fourLocalSizes[fourTransposition[i]] * (ptrdiff_t)position[i]) / (ptrdiff_t)decomposition[i];
  ...
  if (128 % worldSize) {
    ...
    tdd.verify(mem.computeLocalSizes(mGroup4, {128, 128, 128, 128}) ==
    ...
    tdd.verify(mem.computeLocalSizes(mGroup1, {128, 128, 128, 128, 128, 128}) ==
               computeExpectation(mGroup1, {128, 128, 128, 128, 128, 128}));
