...
namespace TempLat
{
  ...
  public:
    ...
    static std::shared_ptr<FFTWHermitianPartners<NDim>> create(const std::array<ptrdiff_t, NDim> &globalSizes)
    {
      return std::make_shared<FFTWHermitianPartners<NDim>>(globalSizes);
    }

    FFTWHermitianPartners(const std::array<ptrdiff_t, NDim> &globalSizes)
        : HermitianPartners<NDim>(globalSizes), mGlobalSizes(globalSizes), mSignConversionMidpoint{}
    {
    ...
    {
      if (globalCoordinate.size() != mGlobalSizes.size())
        throw FFTWHermitianPartnersWrongSizeException("You called qualify with other dimensionality than this layout.",
                                                      globalCoordinate.size(), " != ", mGlobalSizes.size());
     ...
     */
    HermitianRedundancy putHermitianPartner(const std::vector<ptrdiff_t> &globalCoordinate,
    ...
    /** \brief Compute the number of unique / independent real and imaginary floating point values in the FFTW r2c
    ...
    virtual std::string toString() const { return "FFTW HermitianRedundancy describer."; }
