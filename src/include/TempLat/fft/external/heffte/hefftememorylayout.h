#ifndef TEMPLAT_FFT_MEMORYLAYOUTS_HEFFTEMEMORYLAYOUT_H
#define TEMPLAT_FFT_MEMORYLAYOUTS_HEFFTEMEMORYLAYOUT_H
...
#include "TempLat/fft/external/heffte/heffteplanner.h"
#include "TempLat/fft/external/fftw/fftwhermitianpartners.h"

namespace TempLat
{
  MakeException(HEFFTEMemoryLayoutException);

  /** \brief Implements part of FFTLibraryInterface, computes the local memory associated to the global problem, for
   *HEFFTE. Build on this to replace HEFFTE.
   *
   * Unit test: make test-hefftememorylayout
   **/
  template <size_t NDim> class HEFFTEMemoryLayout : public HEFFTEPlanner<NDim>
  {
  public:
    static constexpr bool HEFFTEWITHTRANSPOSITION() { return false; }
