...
namespace TempLat {
    ...
    public:
        ...
        typename std::enable_if<HasGetEval<U>::value, decltype(std::declval<U>().getEval(std::declval<ptrdiff_t>())
        )>::type
        ...
        template<typename U>
        KOKKOS_FORCEINLINE_FUNCTION static
        typename std::enable_if<!HasGetEval<U>::value, decltype(GetValue::get(
            std::declval<U>(), std::declval<ptrdiff_t>()))>::type
        getEval(U &&obj, ptrdiff_t i) {
