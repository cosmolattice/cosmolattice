...
namespace TempLat {
    ...
    public:
        ...
        auto nonabelianclover(const R& Us, Tag<Mu> mu, Tag<Nu> nu){
            ...
            // |     |  |    |
            // v---->   v---->
            /*auto plaq1 =  (Us(mu) * shift(Us(nu),mu) ) * ( dagger(shift(Us(mu),nu)) * dagger(Us(nu)) ); //of course we could call the plaquette here, but we don't for readability.
            auto plaq2 =  (Us(nu) * dagger(shift(shift(Us(mu),-mu),nu))) * ( dagger(shift(Us(nu),-mu)) * shift(Us(mu), -mu));
            auto plaq3 =  (dagger(shift(Us(mu),-mu)) * dagger(shift(shift(Us(nu),-mu),-nu))) * ( shift(shift(Us(mu),-mu),-nu) * shift(Us(nu), -nu));
            auto plaq4 =  (dagger(shift(Us(nu),-nu)) * shift(Us(mu),-nu)) * ( shift(shift(Us(nu),-nu),mu) * dagger(Us(mu)));*/



            auto plaq1 =  (Us(mu) * sh1<Mu>(Us(nu)) ) * ( dag(sh1<Nu>(Us(mu))) * dag(Us(nu)) ); //of course we could call the plaquette here, but we don't for readability.
        ...
        auto B4NA(const R& Us, Tag<1>){
            return nonabelianclover(Us, Tag<2>(), Tag<3>());
        }

        template< typename R>
        auto B4NA(const R& Us, Tag<2>){
            return nonabelianclover(Us, Tag<3>(), Tag<1>());
        }
        template< typename R>
        auto B4NA(const R& Us, Tag<3>){
            return nonabelianclover(Us, Tag<1>(), Tag<2>());
        }
