#ifndef TEMPLAT_LATTICE_ALGEBRA_SPATIALDERIVATIVES_NEUTDIJ_TEST_H
#define TEMPLAT_LATTICE_ALGEBRA_SPATIALDERIVATIVES_NEUTDIJ_TEST_H
...
template <size_t NDim> inline void TempLat::NeutDijTester<NDim>::Test(TempLat::TDDAssertion &tdd)
{
  ...
  // For f(x) = x^2, neutral sum is ((x+1)^2 + (x-1)^2) / (2*dx) = (2x^2 + 2) / (2*dx) = (x^2 + 1) / dx
  {
    ...
    Field<NDim, double> nijsc_sq("nijSC_sq", toolBox);
  ...
  if constexpr (NDim >= 2) {
    ...
    Field<NDim, double> nij_prod_dir1("nij_prod_dir1", toolBox);
