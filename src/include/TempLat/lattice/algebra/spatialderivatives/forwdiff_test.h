#ifndef TEMPLAT_LATTICE_ALGEBRA_SPATIALDERIVATIVES_FORWDIFF_TEST_H
#define TEMPLAT_LATTICE_ALGEBRA_SPATIALDERIVATIVES_FORWDIFF_TEST_H
...
template <size_t NDim> inline void TempLat::ForwDiffTester<NDim>::Test(TempLat::TDDAssertion &tdd)
{
  ...
  constexpr_for<1, NDim + 1, 1>([&](auto _dir) {
    ...
    NDLoop<NDim>(fdsc_view, [&](const auto &...indices) {
      ...
      if (std::abs(expect - val_fdsc) > 1e-14) {
        ...
        sayMPI << "ForwDiff mismatch in dir " << d << " at global (";
        for (size_t i = 0; i < NDim; ++i) {
          sayMPI << global_idx[i];
          if (i < NDim - 1) sayMPI << ", ";
        }
        sayMPI << "): expect = " << expect << ", fdSC = " << val_fdsc << "\n";
      }
    });
    tdd.verify(OK);
  });
  ...
  // For f(x) = x^2, forward difference is ((x+1)^2 - x^2) = 2x + 1
  {
    ...
    NDLoop<NDim>(fdsc_sq_view, [&](const auto &...indices) {
      ...
      if (std::abs(expect - val_fdsc_sq) > 1e-14) {
        ...
        sayMPI << "ForwDiff sq mismatch at global (";
        for (size_t i = 0; i < NDim; ++i) {
          sayMPI << global_idx[i];
          if (i < NDim - 1) sayMPI << ", ";
        }
        sayMPI << "): expect = " << expect << ", fdSC_sq = " << val_fdsc_sq << "\n";
      }
    });
    tdd.verify(OK);
  }
  ...
  if constexpr (NDim >= 2) {
    ...
    NDLoop<NDim>(fd_prod_dir1_view, [&](const auto &...indices) {
      ...
      if (std::abs(expect - val_fd) > 1e-14) {
        ...
        sayMPI << "ForwDiff prod dir1 mismatch at global (";
        for (size_t i = 0; i < NDim; ++i) {
          sayMPI << global_idx[i];
          if (i < NDim - 1) sayMPI << ", ";
        }
        sayMPI << "): expect = " << expect << ", fd = " << val_fd << "\n";
      }
    });
    tdd.verify(OK);
  }
