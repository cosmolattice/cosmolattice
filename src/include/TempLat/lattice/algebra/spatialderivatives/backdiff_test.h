#ifndef TEMPLAT_LATTICE_ALGEBRA_SPATIALDERIVATIVES_BACKDIFF_TEST_H
#define TEMPLAT_LATTICE_ALGEBRA_SPATIALDERIVATIVES_BACKDIFF_TEST_H
...
template <size_t NDim> inline void TempLat::BackDiffTester<NDim>::Test(TempLat::TDDAssertion &tdd)
{
  ...
  auto testDir = [&](auto dirTag) {
    ...
    NDLoop<NDim>(bdsc_view, [&](const auto &...indices) {
      ...
      if (std::abs(expect - val_bdsc) > 1e-14) {
        ...
        sayMPI << "BackDiff mismatch in dir " << d << " at global (";
        for (size_t i = 0; i < NDim; ++i) {
          sayMPI << global_idx[i];
          if (i < NDim - 1) sayMPI << ", ";
        }
        sayMPI << "): expect = " << expect << ", bdSC = " << val_bdsc << "\n";
      }
    });
    tdd.verify(OK);
  };
  ...
  constexpr_for<1, NDim + 1, 1>([&](auto _dir) { testDir(Tag<decltype(_dir)::value>{}); });
  ...
  // For f(x) = x^2, backward difference is (x^2 - (x-1)^2) = 2x - 1
  {
    ...
    NDLoop<NDim>(bdsc_sq_view, [&](const auto &...indices) {
      ...
      if (std::abs(expect - val_bdsc_sq) > 1e-14) {
        ...
        sayMPI << "BackDiff sq mismatch at global (";
        for (size_t i = 0; i < NDim; ++i) {
          sayMPI << global_idx[i];
          if (i < NDim - 1) sayMPI << ", ";
        }
        sayMPI << "): expect = " << expect << ", bdSC_sq = " << val_bdsc_sq << "\n";
      }
    });
    tdd.verify(OK);
  }
  ...
  if constexpr (NDim >= 2) {
    ...
    NDLoop<NDim>(bd_prod_dir1_view, [&](const auto &...indices) {
      ...
      if (std::abs(expect - val_bd) > 1e-14) {
        ...
        sayMPI << "BackDiff prod dir1 mismatch at global (";
        for (size_t i = 0; i < NDim; ++i) {
          sayMPI << global_idx[i];
          if (i < NDim - 1) sayMPI << ", ";
        }
        sayMPI << "): expect = " << expect << ", bd = " << val_bd << "\n";
      }
    });
    tdd.verify(OK);
  }
