...
namespace TempLat {
    ...
    struct ConditionalBinaryGetter : public ConditionalBinaryGetterHelper<OP, S, T,
                ((HasGetMethod<S>::value || HasGetMethod<T>::value) && !HasVectorGetMethod<S>::value && !
                 HasVectorGetMethod<T>::value && !HasStaticGetter<S>::value && !HasStaticGetter<T>::value) &&
                OVERRIDE> {
    };
}
