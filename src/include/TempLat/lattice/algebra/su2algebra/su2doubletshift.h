...
namespace TempLat
{
  ...
  public:
    ...
    SU2DoubletShifter(const R &pR)
        : SU2DoubletUnaryOperator<R>(pR), shiftInd(shift<N...>(mR.SU2DoubletGet(0_c)).getShift())
    {
      shiftString = shift<N...>(mR.SU2DoubletGet(0_c)).getString({N...});
    }

    template <int M> auto SU2DoubletGet(Tag<M> t) { return shift<N...>(mR.SU2DoubletGet(t)); }

    template <int M> auto SU2DoubletGet(Tag<M> t, ptrdiff_t i) { return mR.SU2DoubletGet(t, i + shiftInd); }
