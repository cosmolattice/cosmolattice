...
namespace TempLat {
     ...
     **/
     ...
     class SU2Commutator : public SU2BinaryOperator<R,T> {
     public:
         ...
         auto SU2Get(Tag<1> t)
         {
             return 2 * (mR.SU2Get(3_c) * mT.SU2Get(2_c) - mR.SU2Get(2_c) * mT.SU2Get(3_c));
         }
         auto SU2Get(Tag<2> t)
         {
             return 2 * (mR.SU2Get(1_c) * mT.SU2Get(3_c) - mR.SU2Get(3_c) * mT.SU2Get(1_c));
         }
         auto SU2Get(Tag<3> t)
         {
             return 2 * (mR.SU2Get(2_c) * mT.SU2Get(1_c) - mR.SU2Get(1_c) * mT.SU2Get(2_c));
         }
     ...
     template <typename R, typename T>
     typename std::enable_if<HasSU2Get<R>::value && HasSU2Get<T>::value, SU2Commutator<R,T>>::type
     commutator(const R& r, const T& t)
     {
         return {r,t};
     }
     ...
     commutator(OneType r, const T& t)
     {
         return ZeroType();
     }
     ...
     commutator(const R& r, OneType t)
     {
         return ZeroType();
     }
     ...
     template < typename T>
     typename std::enable_if< HasSU2Get<T>::value, ZeroType>::type
     commutator(ZeroType r, const T& t)
     {
         return ZeroType();
     }
     ...
     template <typename R>
     typename std::enable_if<HasSU2Get<R>::value, ZeroType>::type
     commutator(const R& r, ZeroType t)
     {
         return ZeroType();
     }
