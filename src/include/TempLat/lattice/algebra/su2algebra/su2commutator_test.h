...
inline void TempLat::SU2CommutatorTester::Test(TempLat::TDDAssertion &tdd)
{
  ...
  // Non-trivial test: commutator of two different things.
  {
    result = commutator(su2_1, su2_2);
    ...
    for (size_t i = 0; i < f1view.extent(0); ++i)
      for (size_t j = 0; j < f1view.extent(1); ++j)
        for (size_t k = 0; k < f1view.extent(2); ++k) {
          all_true = all_true && (f1view(i, j, k) == 2.5f);
          all_true = all_true && (f2view(i, j, k) == -3.f);
          all_true = all_true && (f3view(i, j, k) == -3.5f);
