#ifndef COSMOINTERFACE_SU2ALGEBRA_SU2SHIFT_TEST_H
#define COSMOINTERFACE_SU2ALGEBRA_SU2SHIFT_TEST_H
...
inline void TempLat::SU2ShiftTester::Test(TempLat::TDDAssertion &tdd)
{
  ...
  su2(1_c) = getVectorComponent(x, 0);
  su2(2_c) = getVectorComponent(x, 1);
  su2(3_c) = getVectorComponent(x, 2);

  // Test SU2ShifterByOne: shift the SU2 field along each spatial direction
  // and verify that each component is shifted correctly.
  {
    constexpr_for<0, NDim, 1>([&](auto _dir) {
      ...
      constexpr_for<0, 3, 1>([&](auto _comp) {
