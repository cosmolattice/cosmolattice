...
namespace TempLat
{
  ...
  public:
    ...
    static void
    assign(Tup1 &&tup1,
           Tup2 &&tup2) // Can do different children classes with different getter to have different algebras.
    {

      constexpr size_t flatSize = tuple_size<typename std::remove_reference<decltype(tup1)>::type>::value;

      for_in_range<0, flatSize>([&](auto j) { std::get<j>(tup1).onBeforeAssignment(std::get<j>(tup2)); });
      ...
      // auto assigners = make_tuple_tag<0,size>([&](auto j){return
      // MakeRealValueAssigner(std::get<j>(list),std::get<j>(fs));});

      // it.iterate(assigners);

      for_each(tup1, [](auto &x) { x.setGhostsAreStale(); });
    }
