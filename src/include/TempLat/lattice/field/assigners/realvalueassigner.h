...
namespace TempLat
{
   ...
   **/
  template <typename R, typename T> class RealValueAssigner
  {
  public:
    /* Put public methods here. These should change very little over time. */
    RealValueAssigner(R pR, Field<T> fld) : mR(pR), mManager(fld.getMemoryManager()) {}

    void operator()(ptrdiff_t i) { mManager->operator[](i) = GetValue::get(mR, i); }
  ...
  template <typename R, typename T> auto MakeRealValueAssigner(R pR, Field<T> fld)
  {
