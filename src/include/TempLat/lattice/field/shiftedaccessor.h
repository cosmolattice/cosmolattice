...
namespace TempLat
{
   ...
   **/
  template <size_t NDim> class ShiftedAccessor
  {
  public:
    ...
    const ptrdiff_t &getJump() const { return mShifts.memoryJump(); }
    ...
    //               //for ( ptrdiff_t i = 0, iEnd = mShifts.size(); i < iEnd; ++i) it().mCoordinates[i] +=
    //               mShifts.shift(i); return originalOffset;
    //             }
    //
    ...
    void updateJumps(const JumpsHolder<NDim> &jump) { mShifts.setJumps(jump); }

  private:
    /* Put all member variables and private methods here. These may change arbitrarily. */

    ShiftedCoordinatesManager<NDim> mShifts;
