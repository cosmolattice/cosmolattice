...
// File info: Main contributor(s): Adrien Florio,  Year: 2019. Jorge Baeza-Ballesteros, Year: 2023 (big changes to
// eliminate coordinates initialization)
...
namespace TempLat
{
  ...
  public:
    ...
    CoordinatesInitializer(const JumpsHolder<NDim> &jumpsHolder, const LayoutStruct<NDim> &layoutStruct)
        : mJumpsHolder(jumpsHolder), mLayoutStruct(layoutStruct)
    {
    }
                                     ...
                                     const std::array<ptrdiff_t, 2u> &workShare = {{0, 1}})
    {
      ...
      if (thisDim < (ptrdiff_t)NDim - 1) {
        ...
        for (ptrdiff_t i = iStart; i < iEnd && carryOn; ++i) {
          ...
          carryOn =
              carryOn && performIterationInit(offset + i * thisMemoryJump, mOffsets, thisDim + 1, stride, allCarryOn);
