...
namespace TempLat
{
   ...
   **/
  template <size_t NDim> class NPartiteCoordinatesInitializer
  {
  public:
    /* Put public methods here. These should change very little over time. */
    NPartiteCoordinatesInitializer(ptrdiff_t parity, const JumpsHolder<NDim> &jumpsHolder,
                                   const LayoutStruct<NDim> &layoutStruct)
        : mJumpsHolder(jumpsHolder), mLayoutStruct(layoutStruct), mParity(parity)
    {
    }

    void initCoords(const ptrdiff_t &stride, std::vector<std::vector<ptrdiff_t>> &mOffsets,
                    std::vector<std::vector<std::vector<ptrdiff_t>>> &mVectorCoords, ptrdiff_t &mIStart,
  ...
  private:
    ...
    const JumpsHolder<NDim> mJumpsHolder;
    const LayoutStruct<NDim> mLayoutStruct;
                               ...
                               std::vector<ptrdiff_t> &mIEnd)
    {
      ...
      std::vector<ptrdiff_t> vecCoord(NDim);

      performIterationInit(mJumpsHolder.toOrigin(), mOffsets, mVectorCoords, vecCoord, 0, stride, allCarryOn);
    }
    void initCoordinates(const std::array<ptrdiff_t, 2u> &workShare, std::vector<std::vector<ptrdiff_t>> &mOffsets,
                         std::vector<std::vector<std::vector<ptrdiff_t>>> &mVectorCoords, ptrdiff_t &mIStart,
                         std::vector<ptrdiff_t> &mIEnd)
    {
      ptrdiff_t thisMemorySize = 1;
      for (const auto &tmp : mJumpsHolder.getSizesInMemory())
        thisMemorySize *= tmp;
      ...
      for (int j = 0; j < mParity; ++j) {
        mOffsets.push_back({});
        mVectorCoords.push_back(std::vector<std::vector<ptrdiff_t>>(maxOffsets + 1, std::vector<ptrdiff_t>(NDim)));
  ...
  */
    ...
    */
    inline bool performIterationInit(ptrdiff_t offset, std::vector<std::vector<ptrdiff_t>> &mOffsets,
                                     std::vector<std::vector<std::vector<ptrdiff_t>>> &mVectorCoords,
                                     std::vector<ptrdiff_t> &vecCoord, ptrdiff_t thisDim, const ptrdiff_t &stride,
                                     bool &allCarryOn, const std::array<ptrdiff_t, 2u> &workShare = {{0, 1}})
    {
      bool carryOn = true;
      ...
      if (thisDim < NDim - 1) {
        ...
        for (ptrdiff_t i = iStart; i < iEnd && carryOn; ++i) {
          ...
          carryOn = carryOn && performIterationInit(offset + i * thisMemoryJump, mOffsets, mVectorCoords, vecCoord,
                                                    thisDim + 1, stride, allCarryOn);
        ...
        /* funny, get some speed increase by not checking for carry on here: effect -> finish entire rod before breaking
         * the recursive loops. */
        for (ptrdiff_t i = iStart; i < iEnd && carryOn; i += stride) {
          // say << i << ", " << thisMemorySize << ", " << carryOn << "\n";
          mLayoutStruct.putSpatialLocationFromMemoryIndexInto(i, thisDim, vecCoord);
          parityToCheck = 0;
          for (size_t index = 0; index < vecCoord.size(); ++index)
            parityToCheck += vecCoord[index];
          parityToCheck = ((parityToCheck % mParity) + mParity) % mParity;

          mOffsets[parityToCheck][countNum[parityToCheck]] = offset + i * thisMemoryJump;
          mVectorCoords[parityToCheck][mOffsets[parityToCheck][countNum[parityToCheck]]] =
              vecCoord; // Not a typo. Allow to access ijkl... coordinate by passing the offset to array.
