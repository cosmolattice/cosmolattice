...
{
  auto myLittleLambda = [&tdd](int nDim, int parity, int nPoints, int nGhosts) {
    auto toolBox = MemoryToolBox::makeShared<nDim>(nPoints, nGhosts);

    auto testCoord = std::make_shared<NPartiteCoordinates<nDim>>(parity, toolBox->mLayouts.getConfigSpaceJumps(),
