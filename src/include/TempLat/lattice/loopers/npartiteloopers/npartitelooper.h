...
namespace TempLat
{
   ...
   **/
  template <size_t NDim> class NPartiteLooper
  {
  public:
    /* Put public methods here. These should change very little over time. */

    NPartiteLooper(std::shared_ptr<NPartiteCoordinates<NDim>> pCoord, ptrdiff_t pStart, std::vector<ptrdiff_t> pEnd)
        : mCoord(pCoord), mStart(pStart), mEnd(pEnd)
    {
    }
    NPartiteLooper(std::shared_ptr<NPartiteCoordinates<NDim>> pCoord)
        : NPartiteLooper(pCoord, pCoord->getStart(), pCoord->getEnd())
    {
    }
    NPartiteLooper() : mStart(0) {}

    void init(std::shared_ptr<NPartiteCoordinates<NDim>> pCoord)
    {
      mCoord = pCoord;
    ...
    void operator++() { ++mICurrent; }

    inline bool end() const { return mICurrent < mEnd[mPCurrent]; }
    inline void begin() const { mICurrent = mStart; }

    void parityBegin() { mPCurrent = 0; }
    bool parityEnd() const { return mPCurrent < mCoord->getParity(); }

    void parityMove() { ++mPCurrent; }

    size_t next() { return mICurrent++; }

    size_t getEnd() { return mEnd[mPCurrent]; }
    ...
    ptrdiff_t getCurrentParity() const { return mPCurrent; }

  private:
    std::shared_ptr<NPartiteCoordinates<NDim>> mCoord;
