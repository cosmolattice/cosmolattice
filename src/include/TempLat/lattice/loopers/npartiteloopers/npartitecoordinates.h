...
namespace TempLat
{
   ...
   **/
  template <size_t NDim> class NPartiteCoordinates
  {
  public:
    /* Put public methods here. These should change very little over time. */
    NPartiteCoordinates(ptrdiff_t parity, const JumpsHolder<NDim> &jumpsHolder, const LayoutStruct<NDim> &layoutStruct)
        : mParity(parity)
    {
    ...
    ptrdiff_t operator()(size_t lat, ptrdiff_t i) const { return mOffsets[lat][i]; }

    ptrdiff_t &operator()(size_t lat, ptrdiff_t i) { return mOffsets[lat][i]; }
    std::vector<ptrdiff_t> getVec(size_t lat, ptrdiff_t i) const { return mVectorCoords[lat][i]; }
    ptrdiff_t getStart() const { return mIStart; }
    std::vector<ptrdiff_t> getEnd() const { return mIEnd; }

    ptrdiff_t getParity() const { return mParity; }
  ...
  private:
    ...
    template <size_t _NDim> friend class NPartiteLooper;
