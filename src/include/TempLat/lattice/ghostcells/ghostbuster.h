...
namespace TempLat
{
   ...
   *  Then pass any pointer which satisfies for origin layout to the function call operator. And the rest will be
   *history.
   *
   *  ```
  ...
  public:
    ...
    GhostBuster(const JumpsHolder<NDim> &from, const JumpsHolder<NDim> &to, bool verbose = false)
        : mFrom(from), mTo(to), mDirection(mFrom.toOrigin() + mFrom.getJumpsInMemoryOrder()[0] - mTo.toOrigin() -
    ...
    /** \brief overload for passing objects which have a data() and a size() method, like std::vector<T> */
    template <template <typename S> class M, typename T> void operator()(M<T> &obj)
    {
      operator()((T *)obj.data(), obj.size());
    }
#endif
  ...
  private:
    ...
    template <typename T> void bustTheGhosts(T *ptr, ptrdiff_t memSize)
    {
      recursor<T>(mFrom.getSizesInMemory().data(), ptr + mFrom.toOrigin(), mFrom.getJumpsInMemoryOrder().data(),
                  ptr + mTo.toOrigin(), mTo.getJumpsInMemoryOrder().data(), 0, mNDimensions, ptr + memSize);
    }

    template <typename T>
    void recursor(const ptrdiff_t *layout, T *fromPtr, const ptrdiff_t *fromJumps, T *toPtr, const ptrdiff_t *toJumps,
                  ptrdiff_t thisDim, ptrdiff_t nDim, T *endPtr)
    {
      if (thisDim < nDim - 1) {
        ...
        for (ptrdiff_t i = iStart; mDirection * i < mDirection * iEnd; i += di) {
          ...
          recursor<T>(layout + 1, fromPtr + i * (*fromJumps), fromJumps + 1, toPtr + i * (*toJumps), toJumps + 1,
                      thisDim + 1, nDim, endPtr);
        }
      } else { /* move the rods */
               /* who're you gonna call? */
//                say << "forward - Moving " << *layout << " to " << toPtr << " end point " << (toPtr + *layout *
//                sizeof(T))<< " from " << fromPtr << " end point " << (fromPtr + *layout * sizeof(T)) << " Correct? "
