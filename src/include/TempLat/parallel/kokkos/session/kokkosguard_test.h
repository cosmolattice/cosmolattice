...
inline void TempLat::KokkosGuard::Test(TempLat::TDDAssertion& tdd) {
    ...
    if ( KokkosGuard::InstanceCounter() < 1 ) {
        ...
        /* there is an instance of KokkosGuard in the calling main, which is a good thing. Then we can test if the multiple instantiation protection works. */
        tdd.verify( Throws<KokkosGuardInstantiationException>(
                   []() {
                       KokkosGuard guard(0, NULL, true);
                   }
        ) );
