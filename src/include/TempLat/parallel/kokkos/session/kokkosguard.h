...
namespace TempLat
{
  ...
  public:
    ...
    KokkosGuard(int argc, char **argv, bool verbose = false)
  ...
  private:
    ...
    static inline int InstanceCounter(int delta = 0)
    {
      ...
      counter += delta;
      if (counter > 1)
        throw KokkosGuardInstantiationException("Per process, the KokkosGuard can be instantiated only once. This "
