...
{
  char t1[256]("a=5"), t2[256]("  b = 25"), t3[256]("cc=true"), t4[256]("d= 24 45 76 143"),
      t5[256]("t=../../src/tests/fcn/parameters/test_filereader.txt");
  char *argv[6];
  argv[0] = t1;
  argv[1] = t1;
  argv[2] = t2;
  ...
  ParameterParser params(
      6, argv); // we pass number of string plus 1 to argc because in real situation ther eis also the program name
  ...
  tdd.verify(
      AlmostEqual(reduce("(3*(2+2/exp(2+2*8-12+2*4-5^2))-2*8-5)*9/(2+4*4/(2*4))^2^2", params), 12629.17442429954));
  tdd.verify(AlmostEqual(reduce("(3*(2+2/ln(abs(2+2*8-12+2*4-5^2)))-2*8-5)*9/(2+4*4/(2*4))^2^2", params),
                         -0.4393759799339481));
  tdd.verify(
      AlmostEqual(reduce("(3*(2+2/sin(2+2*8-12+2*4-5^2))-2*8-5)*9/(2+4*4/(2*4))^2^2", params), -0.31640418417405747));
  tdd.verify(
      AlmostEqual(reduce("(3*(2+2/cos(2+2*8-12+2*4-5^2))-2*8-5)*9/(2+4*4/(2*4))^2^2", params), 47.13462969905214));
  tdd.verify(
