          ...
          if (tmp.size() > 1) {
            ...
            if (tmp.size() > 1) {
              ...
              if (tmp.size() > 1) {
                res = reduce(tmp[0], parser);
                for (size_t i = 1; i < tmp.size(); ++i)
                  res = std::pow(res, reduce(tmp[i], parser));
                // say << "/";
                // for (std::string x : tmp) say << x;

                return res;
              } else {
                if (unaryReduce(str, res, parser))
                  return res;
                else {
                  // atomic case
  ...
  private:
    ...
    static bool unaryReduce(std::string str, double &res, ParameterParser &parser)
    {
      ...
      if (tmp != std::string::npos) {
        ...
        if (op == "exp")
          res = exp(reduce(copy, parser));
        else if (op == "ln")
          res = log(reduce(copy, parser));
        else if (op == "cos")
          res = std::cos(reduce(copy, parser));
        else if (op == "sin")
          res = std::sin(reduce(copy, parser));
        else if (op == "abs")
          res = fabs(reduce(copy, parser));
        else if (op == "-")
          res = -reduce(copy, parser);
        else
          throw(UnsupportedUnaryOp("Operator " + op + " is not supported by the math parser."));
        return true;
      } else if (str.front() == '-') {
    ...
    static double atomicReduce(std::string str, ParameterParser &params)
    {
      if (str.front() != '\'') {
        ...
        throw(AtomicExpressionNotValid(
            "Your atomic expression " + str +
            " is not valid. It must be either a number, or a mandatory parameter protected by single quotes \'\'"));
    }
