     ...
     class GWsPowerSpectrumMeasurer{
      public:
        ...
        auto gwspowerSpectrum(Model& model, ptrdiff_t N, T kIR)  {
            ...
            T rho = Energies::rho(model);

           if (PSVersion != 3){
                auto fk2 = projectRadiallyGW(model, PSVersion == 1, PRJType, PSType, PSVersion).measure(model, nbins, kMaxBins);
                if (PSType == 2){
                    return Function(ntilde, pow<3>(kIR * ntilde * dx ) / N3 /(8*pow<2>(Constants::pi<T>)*pow(model.aI,2*model.alpha))*pow<2>(model.fStar / Constants::reducedMPlanck<T>)/rho) * fk2;
                }
                else if (PSType == 1){
                    fk2.sumInsteadOfAverage();
                    return Function(ntilde, kIR * ntilde * dx  / pow<5>(N) / (8*(Constants::pi<T>)*pow(model.aI,2*model.alpha))*pow<2>(model.fStar/Constants::reducedMPlanck<T>)/rho) * fk2;
                }
                else{
                ...
                if (PSType == 2){
                    ...
                    return  (pow<3>(kIR *  dx ) / N3 / (8*pow<2>(Constants::pi<T>)*pow(model.aI,2*model.alpha))*pow<2>(model.fStar / Constants::reducedMPlanck<T>)/rho) * fk2;
                }
                else{
                    auto fk2 = projectRadiallyGW(model, false, PRJType, PSType, PSVersion).measure(model,  nbins, kMaxBins);
                    fk2.sumInsteadOfAverage();
                    return (kIR  * dx  / pow<5>(N) / (8*(Constants::pi<T>)*pow(model.aI,2*model.alpha))*pow<2>(model.fStar / Constants::reducedMPlanck<T>)/rho) * fk2;
                }
            }
        }
