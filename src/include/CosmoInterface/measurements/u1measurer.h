        ...
        void measureSpectra(Model& model, T t) {
            ForLoop(k,0,Model::NU1-1,  // Iterates over all gauge fields
                auto magSpecU1 = Total(i, 1, Model::NDim,    // Sums over x, y, z spatial directions
                                       Function(ntilde, pow<2>(ntilde * model.kIR)) * powerSpectrum(model.fldU1(k)(i)));
                auto elSpecU1 = Total(i, 1, Model::NDim, pow(model.aI, 2 * model.alpha - 2) * powerSpectrum(model.piU1(k)(i)));
