...
namespace TempLat
{
   ...
   **/
  template <typename T> class SpectrumGWSaver
  {
  public:
    // Put public methods here. These should change very little over time.

    template <size_t NDim>
    SpectrumGWSaver(FilesManager<NDim> &fm, std::string fn, bool amIRoot, bool appendMode, const RunParameters<T> &rPar,
    ...
    template <size_t NDim>
    SpectrumGWSaver(FilesManager<NDim> &fm, const Field<NDim, T> &fld, bool amIRoot, bool appendMode,
    ...
    template <template <typename> class... Spectra, class Model> void save(T &t, Model &model, Spectra<T>... spectra)
    {
      saverStd->save(
