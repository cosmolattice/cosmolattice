...
namespace TempLat
{
   ...
   **/
  template <typename T> class SpectrumGWSaverStd
  {
  public:
    template <size_t NDim>
    SpectrumGWSaverStd(FilesManager<NDim> &fm, std::string fn, bool amIRoot, bool append, const RunParameters<T> &rPar)
        : mMode(!append ? std::ios_base::out : std::ios_base::app),
          ...
          kIR(rPar.kIR), printHeader(fm.getPrintHeaders()), PSVersion(rPar.powerSpectrumVersion)
    {
    }
    ...
    template <class Model> void save(std::vector<std::shared_ptr<RadialProjectionResult<T>>> arr, T t, Model &model)
    {
      if (printHeader) {
      ...
      (*outputEnergy) << t << " " << (*arr[0]).energy(PSVersion) << " "
                      << (*arr[0]).energy(PSVersion) * Energies::rho(model) << "\n";
