...
namespace TempLat {
    ...
    // the following macros llow the used to define a model simply by specifying a user-defined class, containing the appropriate variable.
...
#define MakeModel(_ModelName, _ModelParsType) AbstractModel<MakeAbstractModelTemplateArgs(_ModelName, _ModelParsType, double)>
    ...
    public:
        ...
        // --> Averages scalar
        T grad2AvI;
        T pi2AvSI, pi2AvSIM, pi2AvI;

        // --> Averages SU2Doublet
        T CSgrad2AvI;
        T CSpi2AvSI, CSpi2AvSIM, CSpi2AvI;

        // --> Averages CS
        T SU2DblGrad2AvI;
        T SU2DblPi2AvSI, SU2DblPi2AvSIM, SU2DblPi2AvI;

        // --> Averages U1
        T U1Mag2AvI;
        T U1pi2AvSI, U1pi2AvSIM, U1pi2AvI;

        // --> Averages SU2
        T SU2Mag2AvI;
        T SU2pi2AvSI, SU2pi2AvSIM, SU2pi2AvI;
        ...
        void setInitialPotentialAndMassesFromPotential()
        {
            ...
            //Compute initial potential at minus half a time step.
            ForLoop(j, 0, Ns-1,fldS(j).getSet(0) -= piS0[j] / fStar / omegaStar * dt / 2.0);
            ForLoop(j, 0, NCs-1,
                    ForLoop(i,0,1,
                            fldCS(j)(i).getSet(0) -= piCS0(j)(i) / fStar / omegaStar * dt / 2.0 ;
                    )
            );
            ForLoop(j, 0, NSU2Doublet-1,
                    ForLoop(i,0,3,
                            fldSU2Doublet(j)(i).getSet(0) -= piSU2Doublet0(j)(i) / fStar /omegaStar * dt / 2.0;
                    )
            );

            pot0SI  = GetValue::get(Potential::potential(static_cast<R&>(*this)),0);
        ...
        void setInitialPotentialFromPotential()
        {
            ...
            //Compute initial potential at t=-dt/2.
            ForLoop(j, 0, Ns-1,fldS(j).getSet(0) -= piS0[j] / fStar / omegaStar * dt / 2.0);
            ForLoop(j, 0, NCs-1,
                    ForLoop(i,0,1,
                            fldCS(j)(i).getSet(0) -= piCS0(j)(i) / fStar / omegaStar * dt / 2.0 ;
			...
			//Saves initial potential at t-dt/2
            pot0SI  = GetValue::get(Potential::potential(static_cast<R&>(*this)),0);
