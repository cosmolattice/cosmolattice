...
namespace TempLat {
    ...
    public:
        ...
        void conjugateGaussianFluctuations(Model& model, Field<T> f, Field<T> p, T mass2, T aDot, T kCutOff) const
        {
            ...
            auto omega = omega_k(k,mass2);  // mode frequency (defined below)
        ...
        static auto omega_k(const Q& k, const R& m2)
        {
            R meff2 = m2 > 0 ? m2 : 0;
