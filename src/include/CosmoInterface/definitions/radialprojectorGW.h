...
// It is a re-implementation of the radial projection which allows to compute the projected
...
namespace TempLat
{
  ...
  public:
    ...
    RadialProjectorGW(Model &model, bool pUseCentralBinValues, int &PRJType, int &PSType, int &PSVersion)
        : mToolBox(model.getToolBox()), mUseBinCentralValues(pUseCentralBinValues), mPRJType(PRJType), mPSType(PSType),
          mPSVersion(PSVersion), gwsProjector(mPRJType)
    {
    }

    RadialProjectionResult<sType> measure(Model &model, ptrdiff_t nLinearBins = -1, sType customRange = -1,
                                          bool excludeOrigin = true)
    {
      ...
      RadialProjectionResult<sType> myResult =
          computeGWSpectrum(model, makeBinComputer(nLinearBins, minValue, customRange), baseWorkSpace, excludeOrigin);
  ...
  private:
    ...
    auto computeGWSpectrum(Model &model, BINCOMPUTETYPE binComputer, RadialProjectionResult<sType> baseWorkSpace,
                           bool excludeOrigin)
    {
      // TODO (Franz)
      /*
            ...
            for (it.begin(); it.end(); ++it) {
              ...
              if ((not excludeOrigin) or (not it.isAtOrigin())) {
                ...
                if (quality != HermitianRedundancy::negativePartner) {
                  ...
                  if (mPSVersion != 3)
                    baseWorkSpace.add(bin, gwsProjector.getProjectionatpoint(model, it), r, weight);
                  else {
                    WaveNumber ntilde(model.getOneField().getToolBox());
                    if (mPSType == 2)
                      baseWorkSpace.add(
                          bin, gwsProjector.getProjectionatpoint(model, it) * GetValue::get(pow<3>(ntilde.norm()),
         it()), r, weight); else baseWorkSpace.add( bin, gwsProjector.getProjectionatpoint(model, it) *
         GetValue::get(ntilde.norm(), it()), r, weight);
                  }
                }
              }
            }
