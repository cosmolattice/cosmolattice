    ...
    public:
        ...
        inline auto Pr(Model& model, Looper& it, Tag<I> i, Tag<J> j) {
            ...
            auto klattice = MakeVector(l, 1, 3,
                                       ...
                                       mType == 2 ? complex<T>(1) - complex<T>(std::cos(2.0 * Constants::pi<T> / N * pVec[l-1]),std::sin(2.0 * Constants::pi<T> / N * pVec[l-1])) :
                                       mType == 3 ? complex<T>(1) - complex<T>(std::cos(2.0 * Constants::pi<T> / N * pVec[l-1]),std::sin(-2.0 * Constants::pi<T> / N * pVec[l-1])) : complex<T>(1.));
