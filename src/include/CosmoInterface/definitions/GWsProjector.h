...
namespace TempLat
{
   ...
   *
   *
   * Unit test: ctest -R test-GWsProjector
   **/
  class GWsProjector
  {
  public:
    // Put public methods here. These should change very little over time.
    GWsProjector(int PRJType) : mType(PRJType) {}

  public:
    /* Function that computes the Projector in base of the choice of lattice momenta given by the forward derivative of
     * eqn 68 in the Art arXiv:2006.15122 */
    ...
    inline auto Pr(Model &model, Looper &it, Tag<I> i, Tag<J> j)
    {
      ...
      // Note that we force the vector to have 3 components instead of Model::NDim so the code compiles for any NDim.
      // The GW module only works for NDim=3. If NDim != 3, the code does not run and returns an error.
      auto klattice =
          MakeVector(l, 1, 3,
                     mType == 1   ? std::sin(2 * Constants::pi<T> * pVec[l - 1] / N)
                     : mType == 2 ? complex<T>(1) - complex<T>(std::cos(2.0 * Constants::pi<T> / N * pVec[l - 1]),
                                                               std::sin(2.0 * Constants::pi<T> / N * pVec[l - 1]))
                     : mType == 3 ? complex<T>(1) - complex<T>(std::cos(2.0 * Constants::pi<T> / N * pVec[l - 1]),
                                                               std::sin(-2.0 * Constants::pi<T> / N * pVec[l - 1]))
                                  : complex<T>(1.));

      T klatticeSquare = Total(k, 1, 3, abs((klattice(k)) * conj(klattice(k))));

      if (i == j) return (complex<T>)(1) - conj(klattice(i)) * (klattice(j)) / klatticeSquare;
    ...
    //             return Total(k, 1, 3, Pr(model, it, i, j) * GetEval::getEval(model.pi_GWtensor(j,k).inFourierSpace(),
    //             it()));
    //         }
    ...
    template <class Model, class Looper> auto projectedGW_complex(Model &model, Looper &it)
    {
      ...
      auto Tr1 = (Pu11 * Pu_s11 + Pu12 * Pu_s21 + Pu13 * Pu_s31) + (Pu21 * Pu_s12 + Pu22 * Pu_s22 + Pu23 * Pu_s32) +
                 (Pu31 * Pu_s13 + Pu32 * Pu_s23 + Pu33 * Pu_s33);
    ...
    template <class Model, class Looper> auto projectedGW_real(Model &model, Looper &it)
    {
      ...
      auto Tr1 = (Pu11 * conj(Pu11) + Pu12 * conj(Pu21) + Pu13 * conj(Pu31)) +
                 (Pu21 * conj(Pu12) + Pu22 * conj(Pu22) + Pu23 * conj(Pu32)) +
                 (Pu31 * conj(Pu13) + Pu32 * conj(Pu23) + Pu33 * conj(Pu33));
      auto Tr2 = (Pu11 + Pu22 + Pu33);

      return pow(model.aI, 2 * (model.alpha - 3)) * abs(Tr1 - .5 * Tr2 * conj(Tr2));
    }

    template <class Model, class Looper> auto getProjectionatpoint(Model &model, Looper &it)
    {
      if (mType == 0)
        return projectedGW_real(model, it);
      else
        return projectedGW_complex(model, it);
      return 0.;
    }
